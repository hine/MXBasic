/******************************************************************************
 ...
 fix 2014/1/21    BOOL _USB_FindClassDriver(), add a argument 
...
#define	CLIENT_DRIVER_HUB					0xfe	// HUB Driver S.Suwa
...
//    volatile DWORD      dBytesSentInFrame;                  // The number of bytes sent during the current frame. Isochronous use only.
    volatile WORD      wBytesSentInFrame;                   // The number of bytes sent during the current frame. 
...
*/
    ...
    typedef struct
    {
        ...
        BYTE  address; // device address S.Suwa
    } USB_EVENT_DATA;
#endif
...
#define _USB_SetNextTransferState(x)     { pCurrentEndpoint[x]->transferState ++; }
...
void                 _USB_SetBDT( BYTE deviceAddress, BYTE direction );
BOOL                 _USB_TransferInProgress( BYTE deviceAddress );

/******************************************************************************
   Multiple transfer in Frame, all device S.Suwa
******************************************************************************/
