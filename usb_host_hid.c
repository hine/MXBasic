/******************************************************************************
  ...
  modified: 2013/11/16 Suwa  http://www.suwa-koubou.jp
...
// *****************************************************************************
// event handler   Suwa
#ifdef USB_HOST_APP_EVENT_HANDLER
    #undef USB_HOST_APP_EVENT_HANDLER
#endif
#define USB_HOST_APP_EVENT_HANDLER USB_HIDDeviceEventHandler

extern BOOL USB_HIDDeviceEventHandler(BYTE address, USB_EVENT event, void *data, DWORD size);
...
static USB_HID_INTERFACE_DETAILS   *pInterfaceDetails[USB_MAX_HID_DEVICES];
static USB_HID_INTERFACE_DETAILS   *pCurrInterfaceDetails[USB_MAX_HID_DEVICES];
                ...
                switch (deviceInfoHID[i].state & SUBSTATE_MASK)
                {
                    case SUBSTATE_WAIT_FOR_ENUMERATION:
                        if (USBHostDeviceStatus( deviceInfoHID[i].ID.deviceAddress ) == USB_DEVICE_ATTACHED)
                        {
                            ...
                            pCurrInterfaceDetails[i] = pInterfaceDetails[i]; // assign current interface to top of list
                        }
                        break;
            ...
            case STATE_GET_REPORT_DSC:
                switch (deviceInfoHID[i].state & SUBSTATE_MASK)
                {
                    case SUBSTATE_SEND_GET_REPORT_DSC:
                        ...
                        if(pCurrInterfaceDetails[i] != NULL) // end of interface list
                        {
                            if(pCurrInterfaceDetails[i]->sizeOfRptDescriptor !=0) // interface must have a Report Descriptor
                            {
                                if((deviceInfoHID[i].rptDescriptor = (BYTE *)USB_MALLOC(pCurrInterfaceDetails[i]->sizeOfRptDescriptor)) == NULL)
                                {
                                ...
                                if (!USBHostIssueDeviceRequest( deviceInfoHID[i].ID.deviceAddress, USB_SETUP_DEVICE_TO_HOST | USB_SETUP_TYPE_STANDARD | USB_SETUP_RECIPIENT_INTERFACE,
                                     USB_REQUEST_GET_DESCRIPTOR, DSC_RPT, pCurrInterfaceDetails[i]->interfaceNumber,pCurrInterfaceDetails[i]->sizeOfRptDescriptor, deviceInfoHID[i].rptDescriptor,
                    ...
                    case SUBSTATE_PARSE_REPORT_DSC:
                        ...
                        deviceInfoHID[i].HIDparserError = _USBHostHID_Parse_Report((BYTE*)deviceInfoHID[i].rptDescriptor , (WORD)pCurrInterfaceDetails[i]->sizeOfRptDescriptor,
                                                                               (WORD)pCurrInterfaceDetails[i]->endpointPollInterval, pCurrInterfaceDetails[i]->interfaceNumber);
                        if(deviceInfoHID[i].HIDparserError)
                        {
                            /* Report Descriptor is flawed , flag error and free memory ,
                               retry by requesting again */
                            #ifdef DEBUG_MODE
                                UART2PrintString("\r\nHID: Error Reported, parser error code=0x");
                    ...
                    case SUBSTATE_PARSING_COMPLETE:
                        ...
                        if (USB_HOST_APP_EVENT_HANDLER(deviceInfoHID[i].ID.deviceAddress, EVENT_HID_RPT_DESC_PARSED, deviceInfoHID[i].rptDescriptor, 0 )) // Suwa
                        {
                        ...
                        else
                        {
                            ...
                            if((pCurrInterfaceDetails[i]->interfaceNumber == (deviceInfoHID[i].noOfInterfaces-1)) &&
                        ...
                        if(deviceInfoHID[i].rptDescriptor != NULL) // Add Suwa
                        USB_FREE_AND_CLEAR(deviceInfoHID[i].rptDescriptor);
                        pCurrInterfaceDetails[i] = pCurrInterfaceDetails[i]->next;
                        if(pCurrInterfaceDetails[i] != NULL)
                        {
...
BYTE USBHostHIDTerminateTransfer( BYTE deviceAddress, BYTE direction, BYTE interfaceNum )
{
    ...
    pCurrInterfaceDetails[i] = pInterfaceDetails[i];
    while((pCurrInterfaceDetails[i] != NULL) && (pCurrInterfaceDetails[i]->interfaceNumber != interfaceNum))
    {
        pCurrInterfaceDetails[i] = pCurrInterfaceDetails[i]->next;
    }
    
    endpoint = pCurrInterfaceDetails[i]->endpointIN;
    if (direction == 0)
    {
        endpoint = pCurrInterfaceDetails[i]->endpointOUT;
    }
...
//            UART2PrintString( "Read, " );
 ...
 //           UART2PrintString( "Write, " );
    ...
    deviceInfoHID[i].endpointDATA      = pCurrInterfaceDetails[i]->endpointIN;
    ...
    if (!direction) // OUT
    {
        deviceInfoHID[i].endpointDATA  = pCurrInterfaceDetails[i]->endpointOUT;
...
{/***
	...
	UART2PrintString("HID: USBHostHID_ApiImportData()\r\n");
	sprintf(buf, "\treportLength=%d\r\n", reportLength); UART2PrintString(buf);
	UART2PrintString("\treport=");
	p = report;
	for(n = 0; n < reportLength;n++){
		sprintf(buf, "%02x", report[n]); UART2PrintString(buf);
	}

	sprintf("\tpDataDetails->reportID=%d\r\n", pDataDetails->reportID); UART2PrintString(buf);
	sprintf("\tpDataDetails->reportLength=%d\r\n", pDataDetails->reportLength); UART2PrintString(buf);
	sprintf("\tpDataDetails->bitOffset=%d\r\n", pDataDetails->bitOffset); UART2PrintString(buf);
	sprintf("\tpDataDetails->bitLength=%d\r\n", pDataDetails->bitLength); UART2PrintString(buf);
	sprintf("\tpDataDetails->count=%d\r\n", pDataDetails->count); UART2PrintString(buf);
***/
}
#endif
 ...
 //                   UART2PrintString( "\r\n" );
                ...
                switch (deviceInfoHID[i].state)
                {
                    case STATE_WAIT_FOR_REPORT_DSC:
                        ...
                        if ((!((HOST_TRANSFER_DATA *)data)->bErrorCode) && (deviceInfoHID[i].bytesTransferred == pCurrInterfaceDetails[i]->sizeOfRptDescriptor ))
                        {
                            /* Invoke HID Parser ,, validate for all the errors in report Descriptor */
//                             deviceInfoHID[i].bytesTransferred = ((HOST_TRANSFER_DATA *)data)->dataCount;
                             deviceInfoHID[i].HIDparserError = _USBHostHID_Parse_Report((BYTE*)deviceInfoHID[i].rptDescriptor , (WORD)pCurrInterfaceDetails[i]->sizeOfRptDescriptor,
                            ...
                            else
                            {
                                ...
                                if(pCurrInterfaceDetails[i] != NULL)
                                {
                                    if(pCurrInterfaceDetails[i]->sizeOfRptDescriptor !=0)
                                    {
                                        if((deviceInfoHID[i].rptDescriptor = (BYTE *)USB_MALLOC(pCurrInterfaceDetails[i]->sizeOfRptDescriptor)) == NULL)
                                        {
                                            #ifdef DEBUG_MODE
...
BOOL USBHostHIDInitialize( BYTE address, DWORD flags, BYTE clientDriverID )
{
    ...
    for (device = 0; (device < USB_MAX_HID_DEVICES) ; device++)
    {
		...
		{
    	#ifdef DEBUG_MODE
        	UART2PrintString( "HID: already Initialize\r\n" );
    	#endif
            return TRUE;
		}
    }
...
//    pCurrInterfaceDetails[device] = pInterfaceDetails[device];

    i = 0;

    #ifdef DEBUG_MODE
        UART2PrintString("HID: Checking descriptor " );
        UART2PutHex( descriptor[i+5] );
        UART2PrintString(" ...\r\n" );
    #endif
    ...
    while (i < ((USB_CONFIGURATION_DESCRIPTOR *)descriptor)->wTotalLength)
    {
        ...
        if (descriptor[i+1] == USB_DESCRIPTOR_INTERFACE) // 0x04 bDescriptorType for an Interface Descriptor
        {
            // See if the interface is a HID interface.
            if ((descriptor[i+5] == DEVICE_CLASS_HID)||(descriptor[i+5] == 0))  // 0x03 HID Class
            {
                ...
                if (deviceInfoHID[device].driverSupported)
                {
                    ...
                    if(pInterfaceDetails[device] == NULL)
                    {
                        pInterfaceDetails[device]       = pNewInterfaceDetails;
                        pCurrInterfaceDetails[device]   = pNewInterfaceDetails;
                        pInterfaceDetails[device]->next = NULL;
                    }
                    else
                    {
                        pCurrInterfaceDetails[device]->next             = pNewInterfaceDetails;
                        pCurrInterfaceDetails[device]                   = pNewInterfaceDetails;
                        pCurrInterfaceDetails[device]->next             = NULL;
                    }

//                       USB_FREE_AND_CLEAR( pNewInterfaceDetails );

                    pCurrInterfaceDetails[device]->interfaceNumber   = descriptor[i+2];

                    // Scan for hid descriptors.
                    i += descriptor[i];
                    if(descriptor[i+1] == DSC_HID) // 0x21 HID Descriptor
                    {
                        if(descriptor[i+5] == 0)
                        {
                        ...
                        else
                        {
                            ...
                            pCurrInterfaceDetails[device]->sizeOfRptDescriptor = ((descriptor[i+7]) |
                                                                          ((descriptor[i+8]) << 8));
...
//
                                pCurrInterfaceDetails[device]->endpointIN           = endpointIN;
                                pCurrInterfaceDetails[device]->endpointOUT          = endpointOUT;
                                pCurrInterfaceDetails[device]->endpointMaxDataSize  = ((descriptor[i+4]) |
                                                                           (descriptor[i+5] << 8));
                                pCurrInterfaceDetails[device]->endpointPollInterval = descriptor[i+6];
...
//    }
    ...
    if(validConfiguration)
    {
            ...
            if (pCurrInterfaceDetails[device]->sizeOfRptDescriptor !=0)
            {
                ...
                if((deviceInfoHID[device].rptDescriptor = (BYTE *)USB_MALLOC(pCurrInterfaceDetails[device]->sizeOfRptDescriptor)) == NULL)
                {
                    #ifdef DEBUG_MODE
            ...
            if (USBHostIssueDeviceRequest( deviceInfoHID[device].ID.deviceAddress, USB_SETUP_DEVICE_TO_HOST | USB_SETUP_TYPE_STANDARD | USB_SETUP_RECIPIENT_INTERFACE,
                 USB_REQUEST_GET_DESCRIPTOR, DSC_RPT, pCurrInterfaceDetails[device]->interfaceNumber, pCurrInterfaceDetails[device]->sizeOfRptDescriptor, deviceInfoHID[device].rptDescriptor,
...
void _USBHostHID_FreeRptDecriptorDataMem(BYTE deviceAddress)
{
    ...
    /* free memory allocated to report descriptor in deviceInfoHID */
       while(pInterfaceDetails[i] != NULL)
        {
            ptempInterface = pInterfaceDetails[i]->next;
            USB_FREE_AND_CLEAR(pInterfaceDetails[i]);
            pInterfaceDetails[i] = ptempInterface;
        }
    }
}
