...
//USB_HID_DEVICE_RPT_INFO deviceRptInfo[USB_MAX_HID_DEVICES];
//USB_HID_ITEM_LIST       itemListPtrs[USB_MAX_HID_DEVICES];
//BYTE*                   parsedDataMem[USB_MAX_HID_DEVICES];
...
#ifdef DEBUG_MODE
{
	...
	UART2PrintString("HID_Parser:_USBHostHID_Parse_Report()\r\n");
	UART2PrintString("   Report length=0x");
	UART2PutHex(lengthOfDescriptor);
	UART2PrintString("\r\n");


	p = hidReportDescriptor;
	n = 0;
	while(n < lengthOfDescriptor){
		UART2PrintString("0x"); UART2PutHex(*p++); n++;
		switch(*(p-1) & 0x03){
		case 0x03:
			UART2PrintString(" 0x"); UART2PutHex(*p++); n++;
			UART2PrintString(" 0x"); UART2PutHex(*p++); n++;
...
#ifdef DEBUG_MODE
    UART2PrintString( "HID_Parser: Memory for Report Descriptor Size=0x" );
...
#ifdef DEBUG_MODE
    UART2PrintString( "HID_Parser: ... 2nd Parse \r\n" );
#endif
