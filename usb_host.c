/******************************************************************************
  ...
  Included USB Hub Driver, Multiple device Support, 2013.4 by Suwa-Koubou
  fix.2013/11/09
      2013/11/16
  update.
      2014/01/13   support mulitiple HUBs (support cascade Hub)
  fix.
      2014/1/21  two functions bug fix.
                 BOOL _USB_FindClassDriver()
                 BOOL _USB_FindDeviceLevelClientDriver()
...
// ****************************************************************************
// *** Multiple Device Support     S.Suwa 
// ****************************************************************************
//#define	USB_HUB_SUPPORT_INCLUDED       // not use this macro

#ifndef USB_HUB_NUMBER_OF_DOWN_STREAM_PORTS         // this macro defined in usb_config.h
	#define	USB_HUB_NUMBER_OF_DOWN_STREAM_PORTS 4	// support default one Hub, it has one up stream port, 4 down stream ports.
#endif

#define USB_MAX_HUB                  (USB_HUB_NUMBER_OF_DOWN_STREAM_PORTS/2)   // maximum number of hubs (Hub has minimum two ports.)
#define	USB_MAX_ADDRESS              (USB_HUB_NUMBER_OF_DOWN_STREAM_PORTS + 1)

static BYTE                          countConfigurations[USB_MAX_ADDRESS+1];              // Count the Configuration Descriptors read during enumeration.
static BYTE                          numCommandTries[USB_MAX_ADDRESS+1];                  // The number of times the current command has been tried.
static BYTE                          numEnumerationTries[USB_MAX_ADDRESS+1];              // The number of times enumeration has been attempted on the attached device.
static volatile WORD                 numTimerInterrupts[USB_MAX_ADDRESS+1];               // The number of milliseconds elapsed during the current waiting period.
static volatile USB_ENDPOINT_INFO   *pCurrentEndpoint[USB_MAX_ADDRESS+1];                 // Pointer to the endpoint currently performing a transfer.
BYTE                                *pCurrentConfigurationDescriptor[USB_MAX_ADDRESS+1];  // Pointer to the current configuration descriptor of the attached device.
BYTE                                *pDeviceDescriptor[USB_MAX_ADDRESS+1];                // Pointer to the Device Descriptor of the attached device.
static volatile WORD                 usbHostState[USB_MAX_ADDRESS+1];                     // aray elements is 0 to USB_MAX_ADDRESS.
static BYTE                         *pEP0Data[USB_MAX_ADDRESS+1];                         // A data buffer for use by EP0.
static USB_DEVICE_INFO               usbDeviceInfo[USB_MAX_ADDRESS+1];                    // A collection of information about the attached device.
volatile WORD                        usbOverrideHostState[USB_MAX_ADDRESS+1];             // Next state machine state, when set by interrupt processing.

// *****************************************************************************
// *****************************************************************************
// Section: HUB driver           S.Suwa 
// *****************************************************************************
// *****************************************************************************
// Hub Task State
#define	HUB_EVENT_READ		               1
#define	HUB_EVENT_READ_WAIT                2
#define	HUB_EVENT_READ_COMPLETE            3
#define	HUB_EVENT_READ_TIME_WAIT           4
#define	HUB_EVENT_READ_TIME_WAITING        5

#define HUB_GET_PORT_STATUS               11
#define HUB_GET_PORT_STATUS_WAIT          12
#define HUB_GET_PORT_STATUS_COMPLETE      13

#define HUB_SET_PORT_FEATURE              21
#define HUB_SET_PORT_FEATURE_WAIT         22
#define HUB_SET_PORT_FEATURE_COMPLETE     23

#define HUB_CLEAR_PORT_FEATURE            31
#define HUB_CLEAR_PORT_FEATURE_WAIT       32
#define HUB_CLEAR_PORT_FEATURE_COMPLETE   33

#define HUB_CLEAR_C_PORT_CONNECTION_BIT   41
#define HUB_CLEAR_C_PORT_ENABLE_BIT       42
#define HUB_CLEAR_C_SUSPEND_BIT           43
#define HUB_CLEAR_C_PORT_OVER_CURRENT_BIT 44
#define HUB_CLEAR_C_PORT_RESET_BIT        45

#define HUB_PORT_STATUS_CHECK             51
...
static BYTE                 HubDeviceAddress[USB_MAX_HUB];
static BYTE                 HubDriverId;        // = CLIENT_DRIVER_HUB
static BYTE                 HubDescriptor[USB_MAX_HUB][8];	// Hub Descriptor
static volatile BYTE        portDeviceLowSpeed;	// 1=low speed, 0=full speed
static volatile BYTE        flgHubAttached;     // set after HUB initialized
USB_ENDPOINT_INFO          *HubEndpoint[USB_MAX_HUB]; // interrupt IN endpoint for notify port change event  
static volatile BYTE        HubState[USB_MAX_HUB], HubNextState[USB_MAX_HUB];
static BYTE                *HubEvent[USB_MAX_HUB]; // notify port change event value 

#define	HubNumberPorts(x)	(HubDescriptor[(x)][2])     // 
#define HubPowerOnGood(x)	(HubDescriptor[(x)][5]*2)	// not use

static volatile BYTE        NumberOfHubs;               // number of USB HUBs
static BYTE                *HubPortAddress[USB_MAX_HUB];
static BYTE                 usbDeviceUse[USB_MAX_ADDRESS+1];

/* *****************************************************************************
    BOOL _USB_HubInit ( BYTE address )
***************************************************************************** */
...
BOOL _USB_HubInit ( BYTE address )
{
	...
	// root hub first connects or reconnects
	if(address == 1){
		NumberOfHubs = 0;
        for(n = 0; n < USB_MAX_HUB; n++) HubDeviceAddress[n] = 0;
		for(n = 0; n < USB_MAX_ADDRESS+1; n++) usbDeviceUse[n] = 0;
	}

	DEBUG_PutString("HUB: HUB Initialized.\r\n");
	DEBUG_PutString(" deviceAddress="); DEBUG_PutHexUINT8(address); DEBUG_PutString("\r\n");

	// too many?
	if(NumberOfHubs >= USB_MAX_HUB){
		DEBUG_PutString("HUB: too mamy HUBs.\r\n");
		return FALSE;
	}

	// search free
	for(n = 0; n < USB_MAX_HUB; n++){
		if(HubDeviceAddress[n] == 0){
			break;
		}
	}

	HubDeviceAddress[n] = address;
    portDeviceLowSpeed = 0;
    HubDriverId = CLIENT_DRIVER_HUB;

	// Hub has only one interfcae, and one IN endpoint.
	HubEndpoint[n] = usbDeviceInfo[address].pInterfaceList->pCurrentSetting->pEndpointList;
	DEBUG_PutString(" endpoint(address=" ); DEBUG_PutHexUINT8(HubEndpoint[n]->bEndpointAddress);
	DEBUG_PutString(", packet Size=");     DEBUG_PutDec(HubEndpoint[n]->wMaxPacketSize);
	DEBUG_PutString(", interval="); DEBUG_PutDec(HubEndpoint[n]->wInterval);
	DEBUG_PutString(")\r\n");

	if(HubEvent[n] != NULL) USB_FREE(HubEvent[n]);
	if((HubEvent[n] = (BYTE *)USB_MALLOC(HubEndpoint[n]->wMaxPacketSize)) == NULL)
	{
        DEBUG_PutString("HUB: Out of Memory.\r\n");
		return FALSE;
	}

	// GET_HUB_DESCRIPTOR
	if( (errorCode = USBHostIssueDeviceRequest(
	    HubDeviceAddress[n], 	  // device address
		USB_SETUP_DEVICE_TO_HOST|USB_SETUP_TYPE_CLASS|USB_SETUP_RECIPIENT_DEVICE, // 0x80|0x20|0x00 bmRequestType
		USB_HUB_GET_DESCRIPTOR,	  // bRequest
		HUB_DESCRIPTOR_TYPE << 8, // wValue
		0,                        // wIndex
		sizeof(HubDescriptor[0]), // wLength
		HubDescriptor[n],         // data
		USB_DEVICE_REQUEST_GET,   // direction
		HubDriverId               // clientDriverID
         )) == USB_SUCCESS)
    {
        while(USBHostTransferIsComplete( HubDeviceAddress[n], 0, &errorCode, &byteCount ) != TRUE)
        {
            //USBHostTasks();
            ;
        }
    ...
    {
        DEBUG_PutString("HUB: Failed GET_HUB_DESCRIPTOR\r\n");
        return FALSE;
    }

	DEBUG_PutString("HUB: HUB_DESCRIPTOR\r\n");
	DEBUG_PutString(" Number of Ports="); DEBUG_PutDec(HubNumberPorts(n)); DEBUG_PutString(" port\r\n");
	DEBUG_PutString(" PowerOnGoodTime="); DEBUG_PutDec(HubPowerOnGood(n)); DEBUG_PutString(" msec\r\n");

	// SET_PORT_FEATURE(PORT_POWER)
    // all Port Power ON. 
    for(port = 1; port <= HubNumberPorts(n); port++)
    {
        if((errorCode = USBHostIssueDeviceRequest(
		    HubDeviceAddress[n],     // device address
            USB_SETUP_HOST_TO_DEVICE|USB_SETUP_TYPE_CLASS|USB_SETUP_RECIPIENT_OTHER, // 0x00|0x20|0x03 bmRequestType
		    USB_HUB_SET_FEATURE,	 // bRequest
            PORT_POWER,              // wValue
            (WORD)port,              // wIndex
            0,                       // wLength
            NULL,                    // data
        ...
        if(errorCode != USB_SUCCESS)
        {
            DEBUG_PutString("HUB: Failed Port Feature PORT_POWER\r\n");
            return FALSE;
        }
    }

	HubState[n] = HUB_EVENT_READ;


	if(HubPortAddress[n] != NULL) USB_FREE(HubPortAddress[n]);
	if((HubPortAddress[n] = (BYTE *)USB_MALLOC(HubNumberPorts(n))) == NULL)
	{
        DEBUG_PutString("HUB: Out of Memory.\r\n");
		return FALSE;
	}

	usbDeviceUse[address] = 1;
	for(m = 1, port = 0; port < HubNumberPorts(n); port++){
		while(usbDeviceUse[m] == 1){
			m++;
		}
		HubPortAddress[n][port] = m;
		usbDeviceUse[m] = 1;
	}
	...
	DEBUG_PutString("HUB: HUB Initialize complete.\r\n");
	return TRUE;
}


/* *****************************************************************************
    void   _USB_HubTasks(void)   State Machine
***************************************************************************** */

static void hub_detached(BYTE address);

void _USB_HubTasks( void )
{
	BYTE	n, m;

	static	BYTE    port[USB_MAX_HUB], portMask[USB_MAX_HUB], selector[USB_MAX_HUB];
	static	WORD	PortStatus[USB_MAX_HUB][2];
	BYTE            errorCode;
	DWORD           byteCount;

  for(m = n = 0; m < NumberOfHubs; m++, n++){
	while(HubDeviceAddress[n] == 0){ // cascade hub already detach
		n++;
	}
	...
	// *************************************************************************
	//    Get Hub Port Event (Interrupt Read)
    // *************************************************************************

	case HUB_EVENT_READ:
		memset(HubEvent[n], 0, HubEndpoint[n]->wMaxPacketSize);
        if(USBHostRead( HubDeviceAddress[n], HubEndpoint[n]->bEndpointAddress, HubEvent[n],
                           HubEndpoint[n]->wMaxPacketSize ) == USB_SUCCESS )
        {
            HubState[n] = HUB_EVENT_READ_WAIT;
        }
        else
        {
			// to Time Wait 
            USBHostClearEndpointErrors( HubDeviceAddress[n], HubEndpoint[n]->bEndpointAddress );
            HubState[n] = HUB_EVENT_READ_TIME_WAIT; 
        }
        break;

    case HUB_EVENT_READ_WAIT:
        if(USBHostTransferIsComplete( HubDeviceAddress[n], HubEndpoint[n]->bEndpointAddress,
                                &errorCode, &byteCount ) == TRUE )
        {
            // Transfer Success
            if(errorCode == USB_SUCCESS)
            {
                HubState[n] = HUB_EVENT_READ_COMPLETE;
            }
            else
            // Transfer Fail
            {
    ...
    case HUB_EVENT_READ_COMPLETE:
		// check change port
        for(port[n] = 1; port[n] <= HubNumberPorts(n); port[n]++)
        {
            portMask[n] = (1 << (port[n] % 8));
            if(HubEvent[n][port[n]/8] & portMask[n]) break;
        }
        // found port
        if(port[n] <= HubNumberPorts(n))
        {
		    DEBUG_PutString("HUB: port change event! port="); DEBUG_PutHexUINT8(port[n]); DEBUG_PutString("\r\n");
			HubState[n] = HUB_GET_PORT_STATUS;
        }
        else
        { // to time wait state, next read
            HubState[n] = HUB_EVENT_READ_TIME_WAIT;
        }
        break;

    case HUB_EVENT_READ_TIME_WAIT:
		HubState[n] = HUB_EVENT_READ;
		break;
    ...
    case HUB_GET_PORT_STATUS:
        if(USBHostIssueDeviceRequest(
		    HubDeviceAddress[n],     // device address
            USB_SETUP_DEVICE_TO_HOST|USB_SETUP_TYPE_CLASS|USB_SETUP_RECIPIENT_OTHER, // 0x80|0x20|0x03 bmRequestType
		    USB_HUB_GET_STATUS,	     // bRequest
            0,                       // wValue
		    (WORD)port[n],           // wIndex
            4,                       // wLength
            (BYTE *)PortStatus[n],   // data
            USB_DEVICE_REQUEST_GET,  // direction
		    HubDriverId              // clientDriverID
                ) == USB_SUCCESS)
        {
            HubState[n] = HUB_GET_PORT_STATUS_WAIT;
        }
        break;

    case HUB_GET_PORT_STATUS_WAIT:
        if(USBHostTransferIsComplete( HubDeviceAddress[n], 0, &errorCode, &byteCount ) == TRUE )
        {
            // Transfer Success
            if(errorCode == USB_SUCCESS)
            {
                HubState[n] = HUB_GET_PORT_STATUS_COMPLETE;
            }
            else
            // Transfer Fail
            {
                USBHostClearEndpointErrors( HubDeviceAddress[n], 0);
                HubState[n] = HUB_GET_PORT_STATUS; // retry GET request
		    }
        }
        break;

    case HUB_GET_PORT_STATUS_COMPLETE:
		//HubState[n] = HUB_CLEAR_C_PORT_CONNECTION_BIT;
        //break;
        // not break;
   ...
   // **************************************************************************
    ...
    case HUB_CLEAR_C_PORT_CONNECTION_BIT:
	    if(PortStatus[n][1] & C_PORT_CONNECTION_BIT)
        {
            DEBUG_PutString("HUB: Changed PORT_CONNECTION\r\n"); 
            HubNextState[n] = HUB_CLEAR_C_PORT_ENABLE_BIT;
            selector[n] = C_PORT_CONNECTION;
            HubState[n] = HUB_CLEAR_PORT_FEATURE;
            break;
        }
		// not break
        // fall threw

    case HUB_CLEAR_C_PORT_ENABLE_BIT:
        if(PortStatus[n][1] & C_PORT_ENABLE_BIT)
        {
            DEBUG_PutString("HUB: Changed PORT_ENABLE\r\n"); 
            HubNextState[n] = HUB_CLEAR_C_SUSPEND_BIT;
            selector[n] = C_PORT_ENABLE;
            HubState[n] = HUB_CLEAR_PORT_FEATURE;
            break;
        }
        // not break;

    case HUB_CLEAR_C_SUSPEND_BIT:
        if(PortStatus[n][1] & C_PORT_SUSPEND_BIT)
        {
            DEBUG_PutString("HUB: Changed PORT_SUSPEND\r\n"); 
            HubNextState[n] = HUB_CLEAR_C_PORT_OVER_CURRENT_BIT;
            selector[n] = C_PORT_SUSPEND;
            HubState[n] = HUB_CLEAR_PORT_FEATURE;
            break;
        }
        // not break;

    case HUB_CLEAR_C_PORT_OVER_CURRENT_BIT:
        if(PortStatus[n][1] & C_PORT_OVER_CURRENT_BIT)
        {
            DEBUG_PutString("HUB: Changed PORT_OVER_CURRENT\r\n"); 
            HubNextState[n] = HUB_CLEAR_C_PORT_RESET_BIT;
            selector[n] = C_PORT_OVER_CURRENT;
            HubState[n] = HUB_CLEAR_PORT_FEATURE;
            break;
        } 
        // not break;

    case HUB_CLEAR_C_PORT_RESET_BIT:
        if(PortStatus[n][1] & C_PORT_RESET_BIT)
        {
            DEBUG_PutString("HUB: Changed PORT_RESET\r\n"); 
            HubNextState[n] = HUB_PORT_STATUS_CHECK;
            selector[n] = C_PORT_RESET;
            HubState[n] = HUB_CLEAR_PORT_FEATURE;
            break;
        } 
        // not break;
    ...
    case HUB_PORT_STATUS_CHECK:
        ...
        if(PortStatus[n][1] & C_PORT_CONNECTION_BIT)
        {
            if(!(PortStatus[n][0] & PORT_CONNECTION_BIT))
            {
                // port device detached!!
                DEBUG_PutString("HUB: port Device detached\r\n"); 
                //usbHostState[port[n]+HubPortOffset[n]] = STATE_DETACHED;
                hub_detached(HubPortAddress[n][port[n]-1]);
                HubState[n] = HUB_EVENT_READ_TIME_WAIT;
                break;
            }

	        if((PortStatus[n][0] & (PORT_CONNECTION_BIT|PORT_ENABLE_BIT)) ==(PORT_CONNECTION_BIT|PORT_ENABLE_BIT)) 
            {
                // port device attached !!
                DEBUG_PutString("HUB: port Device attached\r\n"); 
                portDeviceLowSpeed = 0;
                usbHostState[HubPortAddress[n][port[n]-1]] = STATE_ATTACHED;
                HubState[n] = HUB_EVENT_READ_TIME_WAIT;
                if(PortStatus[n][0] & PORT_LOW_SPEED_BIT) portDeviceLowSpeed = 1;
                 break;
            }

            if(PortStatus[n][0] & PORT_CONNECTION_BIT)
            {
                DEBUG_PutString("HUB: port Device connected\r\n"); 
                HubNextState[n] = HUB_EVENT_READ_TIME_WAIT;
                selector[n] = PORT_RESET;
                HubState[n] = HUB_SET_PORT_FEATURE;
                break;
            }
        }
        ...
        {
            // to Power Down        
            DEBUG_PutString("HUB: port Device over current\r\n"); 
            HubNextState[n] = HUB_EVENT_READ_TIME_WAIT;
            selector[n] = PORT_POWER;
            HubState[n] = HUB_CLEAR_PORT_FEATURE;
            //usbHostState[port[n]+HubPortOffset[n]] = STATE_DETACHED;
            hub_detached(HubPortAddress[n][port[n]-1]);
        ...
        // not support CHANGE PORT SUSPEND
        HubState[n] = HUB_EVENT_READ_TIME_WAIT;
        break;

	// *************************************************************************
	//    SET_PORT_FEATURE  (port, selector)
    // *************************************************************************
	...
	case HUB_SET_PORT_FEATURE:
        ...
        {
            HubState[n] = HUB_SET_PORT_FEATURE_WAIT;
        }
        break;


	case HUB_SET_PORT_FEATURE_WAIT:
        if(USBHostTransferIsComplete( HubDeviceAddress[n], 0, &errorCode, &byteCount ) == TRUE )
        {
            // Transfer Success
            if(errorCode == USB_SUCCESS)
            {
                HubState[n] = HUB_SET_PORT_FEATURE_COMPLETE;
            }
            else
            // Transfer Fail
            {
                USBHostClearEndpointErrors( HubDeviceAddress[n], 0);
                HubState[n] = HUB_SET_PORT_FEATURE; // retry SET request
		    }
        }
        break;

	case HUB_SET_PORT_FEATURE_COMPLETE:
		HubState[n] = HubNextState[n];
		break;


	// *************************************************************************
	//    CLEAR_PORT_FEATURE  (port, selector)
    // *************************************************************************
	...
	case HUB_CLEAR_PORT_FEATURE:
        if(USBHostIssueDeviceRequest(
		    HubDeviceAddress[n],     // device address
            USB_SETUP_HOST_TO_DEVICE|USB_SETUP_TYPE_CLASS|USB_SETUP_RECIPIENT_OTHER, // 0x00|0x20|0x03 bmRequestType
		    USB_HUB_CLEAR_FEATURE,	 // bRequest
            selector[n],             // wValue
		    (WORD)port[n],           // wIndex
            0,                       // wLength
            NULL,                    // data
        ...
        {
            HubState[n] = HUB_CLEAR_PORT_FEATURE_WAIT;
        }
        break;

	case HUB_CLEAR_PORT_FEATURE_WAIT:
        if(USBHostTransferIsComplete( HubDeviceAddress[n], 0, &errorCode, &byteCount ) == TRUE )
        {
            // Transfer Success
            if(errorCode == USB_SUCCESS)
            {
                HubState[n] = HUB_CLEAR_PORT_FEATURE_COMPLETE;
            }
            else
            // Transfer Fail
            {
                USBHostClearEndpointErrors( HubDeviceAddress[n], 0);
                HubState[n] = HUB_CLEAR_PORT_FEATURE; // retry SET request
		    }
        }
        break;

	case HUB_CLEAR_PORT_FEATURE_COMPLETE:
		HubState[n] = HubNextState[n];
        break;
...
static void hub_detached(BYTE address)
{
    ...
    usbHostState[address] = STATE_DETACHED;

	// detache device is a cascade hub.
    for(m = n = 0; m < NumberOfHubs; m++, n++){
		while(HubDeviceAddress[n] == 0){
			n++;
		}

		if(HubDeviceAddress[n] == address){
			for(port = 0; port < HubNumberPorts(n); port++){
				hub_detached(HubPortAddress[n][port]);
				usbDeviceUse[HubPortAddress[n][port]] = 0;
			}
			HubDeviceAddress[n] = 0;
			NumberOfHubs--;
			return;
		}
	}
...
//	DEBUG_PutString("HOST: USBHostClearEndpointErrors(deviceAddress=0x");
//	DEBUG_PutHexUINT8(deviceAddress); DEBUG_PutString(", endpoint=0x");
//	DEBUG_PutHexUINT8(endpoint); DEBUG_PutString(")\r\n");

    // Find the required device
    if (deviceAddress != usbDeviceInfo[deviceAddress].deviceAddress)
    {
        return USB_UNKNOWN_DEVICE;
    }

    ep = _USB_FindEndpoint( deviceAddress, endpoint );
...
BOOL    USBHostDeviceSpecificClientDriver( BYTE deviceAddress )
{
	DEBUG_PutString("HOST: USBHostDeviceSpecificClientDriver(deviceAddress=0x");
	DEBUG_PutHexUINT8(deviceAddress); DEBUG_PutString(")\r\n");

	if (deviceAddress != usbDeviceInfo[deviceAddress].deviceAddress) return FALSE;  
	return usbDeviceInfo[deviceAddress].flags.bfUseDeviceClientDriver;
}
...
BYTE USBHostDeviceStatus( BYTE deviceAddress )
{
//	DEBUG_PutString("HOST: USBHostDeviceStatus(deviceAddress=0x");
//	DEBUG_PutHexUINT8(deviceAddress); DEBUG_PutString(")\r\n");
    ...
    if ((usbHostState[deviceAddress] & STATE_MASK) == STATE_DETACHED)
    {
        return USB_DEVICE_DETACHED;
    }

    if ((usbHostState[deviceAddress] & STATE_MASK) == STATE_RUNNING)
    {
        if ((usbHostState[deviceAddress] & SUBSTATE_MASK) == SUBSTATE_SUSPEND_AND_RESUME)
        {
    ...
    if ((usbHostState[deviceAddress] & STATE_MASK) == STATE_HOLDING)
    {
        return usbDeviceInfo[deviceAddress].errorCode;
    }
...
BOOL USBHostInit(  BYTE deviceAddress  )
{
  DEBUG_PutString("HOST: USBHostInit("); DEBUG_PutDec(deviceAddress); DEBUG_PutString(")\r\n");

// fix.2013/11/16 Suwa, delete for() loop
    ...
    if (usbDeviceInfo[deviceAddress].pEndpoint0 == NULL)
    {
        if ((usbDeviceInfo[deviceAddress].pEndpoint0 = (USB_ENDPOINT_INFO*)USB_MALLOC( sizeof(USB_ENDPOINT_INFO) )) == NULL)
        {
            DEBUG_PutString( "HOST: Cannot allocate for endpoint 0.\r\n" );
            return FALSE;
        }
        DEBUG_PutString( "HOST: Allocate for endpoint 0. deviceAddress=" );
        DEBUG_PutDec(deviceAddress); DEBUG_PutString(")\r\n");

        usbDeviceInfo[deviceAddress].pEndpoint0->next = NULL;
    }
    else
    {
        _USB_FreeMemory(deviceAddress);
    }

    // Initialize other variables.
    pCurrentEndpoint[deviceAddress]                   = usbDeviceInfo[deviceAddress].pEndpoint0;
    usbHostState[deviceAddress]                       = STATE_DETACHED;
    usbOverrideHostState[deviceAddress]               = NO_STATE;
    usbDeviceInfo[deviceAddress].deviceAddressAndSpeed = 0;
    usbDeviceInfo[deviceAddress].deviceAddress        = 0;

    countConfigurations[deviceAddress] = 0;              // Count the Configuration Descriptors read during enumeration.
    numCommandTries[deviceAddress] = 0;                  // The number of times the current command has been tried.
    numEnumerationTries[deviceAddress] = 0;              // The number of times enumeration has been attempted on the attached device.
    numTimerInterrupts[deviceAddress] = 0;               // The number of milliseconds elapsed during the current waiting period.
    pCurrentEndpoint[deviceAddress] = NULL;                 // Pointer to the endpoint currently performing a transfer.
    pCurrentConfigurationDescriptor[deviceAddress] = NULL;  // Pointer to the current configuration descriptor of the attached device.
    pDeviceDescriptor[deviceAddress] = NULL;                // Pointer to the Device Descriptor of the attached device.

  if(deviceAddress <= 1){ // at All initilize timing
    ...
    flgHubAttached = 0; // HUB Attached flag
  }
...
BOOL USBHostIsochronousBuffersCreate( ISOCHRONOUS_DATA * isocData, BYTE numberOfBuffers, WORD bufferSize )
{
	...
	DEBUG_PutString("HOST: USBHostIsochronousBuffersCreate(*isocData, numberOfBuffers=");
	DEBUG_PutDec(numberOfBuffers); DEBUG_PutString(", bufferSize=0x"); 
    DEBUG_PutHexUINT16(bufferSize); DEBUG_PutString(",)\r\n");
...
void USBHostIsochronousBuffersDestroy( ISOCHRONOUS_DATA * isocData, BYTE numberOfBuffers )
{
	...
	DEBUG_PutString("HOST: USBHostIsochronousBuffersDestroy(*isocData, numberOfBuffers=");
	DEBUG_PutDec(numberOfBuffers); DEBUG_PutString(")\r\n");
...
void USBHostIsochronousBuffersReset( ISOCHRONOUS_DATA * isocData, BYTE numberOfBuffers )
{
	...
	DEBUG_PutString("HOST: USBHostIsochronousBuffersReset(*isocData, numberOfBuffers=");
	DEBUG_PutDec(numberOfBuffers); DEBUG_PutString(")\r\n");
...
BYTE USBHostIssueDeviceRequest( BYTE deviceAddress, BYTE bmRequestType, BYTE bRequest,
            ...
            BYTE clientDriverID )
{
/*	DEBUG_PutString("HOST: USBHostIssueDeviceRequest(deviceAddress=0x"); DEBUG_PutHexUINT8(deviceAddress);
	DEBUG_PutString(", bmRequestType=0x"); DEBUG_PutHexUINT8(bmRequestType);
	DEBUG_PutString(", bRequest=0x"); DEBUG_PutHexUINT8(bRequest);
	DEBUG_PutString(", wValue=0x"); DEBUG_PutHexUINT16(wValue);
	DEBUG_PutString(", wIndex=0x"); DEBUG_PutHexUINT16(wIndex);
	DEBUG_PutString(", wLength=0x"); DEBUG_PutHexUINT16(wLength);
	DEBUG_PutString(", *data, dataDirection=0x"); DEBUG_PutHexUINT8(dataDirection);
	DEBUG_PutString(", clientDriverID=0x"); DEBUG_PutHexUINT8(clientDriverID);
	DEBUG_PutString(")\r\n");
*/
    // Find the required device
    if (deviceAddress != usbDeviceInfo[deviceAddress].deviceAddress)
    ...
    if (!usbDeviceInfo[deviceAddress].pEndpoint0->status.bfTransferComplete)
    {
...
//    }
    ...
    if (bRequest == USB_REQUEST_SET_INTERFACE)
    {
        ...
        // interface setting.
        pInterface = usbDeviceInfo[deviceAddress].pInterfaceList;
    ...
    if ((bRequest == USB_REQUEST_CLEAR_FEATURE) && (wValue == USB_FEATURE_ENDPOINT_HALT))
    {
        switch(bmRequestType)
        {
            ...
            case 0x01:
            case 0x02:
                _USB_ResetDATA0( deviceAddress, (BYTE)wIndex );
                break;
            default:
                break;
        }
    ...
    pEP0Data[deviceAddress][0] = bmRequestType;
    pEP0Data[deviceAddress][1] = bRequest;
    pEP0Data[deviceAddress][2] = wValue & 0xFF;
    pEP0Data[deviceAddress][3] = (wValue >> 8) & 0xFF;
    pEP0Data[deviceAddress][4] = wIndex & 0xFF;
    pEP0Data[deviceAddress][5] = (wIndex >> 8) & 0xFF;
    pEP0Data[deviceAddress][6] = wLength & 0xFF;
    pEP0Data[deviceAddress][7] = (wLength >> 8) & 0xFF;

    // Set up the client driver for the event.
    usbDeviceInfo[deviceAddress].pEndpoint0->clientDriver = clientDriverID;

    if (dataDirection == USB_DEVICE_REQUEST_SET)
    {
        // We are doing a SET command that requires data be sent.
        _USB_InitControlWrite( usbDeviceInfo[deviceAddress].pEndpoint0, pEP0Data[deviceAddress],8, data, wLength );
    }
    else
    {
        // We are doing a GET request.
        _USB_InitControlRead( usbDeviceInfo[deviceAddress].pEndpoint0, pEP0Data[deviceAddress], 8, data, wLength );
    }
...
/*	DEBUG_PutString("HOST: USBHostRead(deviceAddress=0x");	DEBUG_PutHexUINT8(deviceAddress);
	DEBUG_PutString(", endpoint=0x"); DEBUG_PutHexUINT8(endpoint);
	DEBUG_PutString(", *pData, size=0x"); DEBUG_PutHexUINT32(size);
	DEBUG_PutString(")\r\n");
*/
    // Find the required device
    if (deviceAddress != usbDeviceInfo[deviceAddress].deviceAddress)
...
BYTE USBHostResetDevice( BYTE deviceAddress )
{
	DEBUG_PutString("HOST: USBHostResetDevice(deviceAddress="); DEBUG_PutDec(deviceAddress); DEBUG_PutString(")\r\n");
...
BYTE USBHostResumeDevice( BYTE deviceAddress )
{
	DEBUG_PutString("HOST: USBHostResumeDevice(deviceAddress="); DEBUG_PutDec(deviceAddress); DEBUG_PutString(")\r\n");
    ...
    if (usbHostState[deviceAddress] != (STATE_RUNNING | SUBSTATE_SUSPEND_AND_RESUME | SUBSUBSTATE_SUSPEND))
    {
        return USB_ILLEGAL_REQUEST;
    }

    // Advance the state machine to issue resume signalling.
    _USB_SetNextSubSubState(deviceAddress);
...
BYTE USBHostSetDeviceConfiguration( BYTE deviceAddress, BYTE configuration )
{
	DEBUG_PutString("HOST: USBHostSetDeviceConfiguration(deviceAddress="); DEBUG_PutDec(deviceAddress);
	DEBUG_PutString(", configuration="); DEBUG_PutDec(configuration); DEBUG_PutString(")\r\n");
    ...
    if (_USB_TransferInProgress(deviceAddress))
    {
        return USB_BUSY;
    }

    // Set the new device configuration.
    usbDeviceInfo[deviceAddress].currentConfiguration = configuration;
    ...
    // to a client driver.
    usbDeviceInfo[deviceAddress].pEndpoint0->clientDriver = CLIENT_DRIVER_HOST;
    ...
    usbHostState[deviceAddress] = STATE_CONFIGURING | SUBSTATE_SELECT_CONFIGURATION;
    return USB_SUCCESS;
}
...
BYTE USBHostSetNAKTimeout( BYTE deviceAddress, BYTE endpoint, WORD flags, WORD timeoutCount )
{
	...
	DEBUG_PutString("HOST: USBHostSetNAKTimeout(deviceAddress="); DEBUG_PutDec(deviceAddress);
	DEBUG_PutString(", endpoint=0x"); DEBUG_PutHexUINT8(endpoint);
    DEBUG_PutString(", flags=0x"); DEBUG_PutHexUINT16(flags);
	DEBUG_PutString(", timeoutCount=0x"); DEBUG_PutHexUINT16(timeoutCount);
	DEBUG_PutString(")\r\n");
...
void USBHostShutdown( BYTE deviceAddress )
{
  DEBUG_PutString("HOST: USBHostShutdown(deviceAddress="); DEBUG_PutDec(deviceAddress); DEBUG_PutString(")\r\n");
...
// fix.2013/11/16 Suwa, delete for() loop
    ...
    // overcurrent situation.
        ...
        if (!USBOTGHnpIsActive())
        {
            ...
            if (usbDeviceInfo[deviceAddress].deviceAddress)
            {
                USB_VBUS_POWER_EVENT_DATA   powerRequest;

                powerRequest.port = 0;  // Currently was have only one port.

                USB_HOST_APP_EVENT_HANDLER( usbDeviceInfo[deviceAddress].deviceAddress,
                                            EVENT_VBUS_RELEASE_POWER,
                                            &powerRequest,
                                            sizeof(USB_VBUS_POWER_EVENT_DATA)
                                          );
                _USB_NotifyClients( usbDeviceInfo[deviceAddress].deviceAddress,
                                    EVENT_DETACH,
...
BYTE USBHostSuspendDevice( BYTE deviceAddress )
{
    DEBUG_PutString("HOST: USBHostSuspendDevice(deviceAddress="); DEBUG_PutDec(deviceAddress); DEBUG_PutString(")\r\n");
    ...
    if (usbHostState[deviceAddress] != (STATE_RUNNING | SUBSTATE_NORMAL_RUN))
    {
        return USB_ILLEGAL_REQUEST;
    }
  ...
  if(deviceAddress <= 1){
    ...
    usbHostState[deviceAddress] = STATE_RUNNING | SUBSTATE_SUSPEND_AND_RESUME | SUBSUBSTATE_SUSPEND;
...
void USBHostTasks( void )
{
    ...
    BYTE                        temp;
    USB_VBUS_POWER_EVENT_DATA   powerRequest;
    BYTE                        deviceAddress;   // S.Suwa

//  DEBUG_PutChar('<');
//  DEBUG_PutHexUINT8( usbHostState>>8 );
//  DEBUG_PutHexUINT8( usbHostState & 0xff );
//  DEBUG_PutChar('-');
//  DEBUG_PutHexUINT8( pCurrentEndpoint->transferState );
//  DEBUG_PutChar('>');
    ...
    // If the ATTACH bit is 0, we have detached.
            ...
            if (((usbHostState[1] & STATE_MASK) != STATE_DETACHED) && !U1IRbits.ATTACHIF)
            {
                DEBUG_PutChar( '>' );
                DEBUG_PutChar( ']' );

                usbHostState[1] = STATE_DETACHED;
            }
        #endif
    #endif // __PIC32MX__
  ...
  // *** Hub Connect, Multiple Device Support S.Suwa *********************
  // Call Hub Function
  if(flgHubAttached)
  {
      _USB_HubTasks();
    ...
    switch (usbHostState[deviceAddress] & STATE_MASK)
    {
        case STATE_DETACHED:
            switch (usbHostState[deviceAddress] & SUBSTATE_MASK)
            {
                case SUBSTATE_INITIALIZE:
                    ...
                    usbDeviceInfo[deviceAddress].pEndpoint0->next                         = NULL;
                    usbDeviceInfo[deviceAddress].pEndpoint0->status.val                   = 0x00;
                    usbDeviceInfo[deviceAddress].pEndpoint0->status.bfUseDTS              = 1;
                    usbDeviceInfo[deviceAddress].pEndpoint0->status.bfTransferComplete    = 1;    // Initialize to success to allow preprocessing loops.
                    usbDeviceInfo[deviceAddress].pEndpoint0->status.bfNAKTimeoutEnabled   = 1;    // So we can catch devices that NAK forever during enumeration
                    usbDeviceInfo[deviceAddress].pEndpoint0->timeoutNAKs                  = USB_NUM_CONTROL_NAKS;
                    usbDeviceInfo[deviceAddress].pEndpoint0->wMaxPacketSize               = 64;
                    usbDeviceInfo[deviceAddress].pEndpoint0->dataCount                    = 0;    // Initialize to 0 since we set bfTransferComplete.
                    usbDeviceInfo[deviceAddress].pEndpoint0->bEndpointAddress             = 0;
                    usbDeviceInfo[deviceAddress].pEndpoint0->transferState                = TSTATE_IDLE;
                    usbDeviceInfo[deviceAddress].pEndpoint0->bmAttributes.bfTransferType  = USB_TRANSFER_TYPE_CONTROL;
                    usbDeviceInfo[deviceAddress].pEndpoint0->clientDriver                 = CLIENT_DRIVER_HOST;

                    // Initialize any device specific information.
                    numEnumerationTries[deviceAddress]                 = USB_NUM_ENUMERATION_TRIES;
                    usbDeviceInfo[deviceAddress].currentConfiguration  = 0; // Will be overwritten by config process or the user later
                    usbDeviceInfo[deviceAddress].attributesOTG         = 0;
                    usbDeviceInfo[deviceAddress].deviceAddressAndSpeed = 0;
                    usbDeviceInfo[deviceAddress].flags.val             = 0;
                    usbDeviceInfo[deviceAddress].pInterfaceList        = NULL;
                if(deviceAddress == 1){  
                    ...
                    _USB_SetNextSubState(deviceAddress);
                    break;
                ...
                if(deviceAddress == 1){
                   ...
                   // HUB detect attached device. HUB port power on. 
                }
                    break;
                ...
                case SUBSTATE_TURN_ON_POWER:
                  ...
                  { 
                    ...
                    else
                    {
                        ...
                        usbHostState[deviceAddress] = STATE_DETACHED | SUBSTATE_WAIT_FOR_POWER;
                    }
                  }
                  else
                  {
                    // HUB port connect device
                    _USB_SetNextSubState(deviceAddress);
                  } 
                    break;
        ...
        case STATE_ATTACHED:
            switch (usbHostState[deviceAddress] & SUBSTATE_MASK)
            {
                case SUBSTATE_SETTLE:
                    ...
                    switch (usbHostState[deviceAddress] & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_START_SETTLING_DELAY:
                            ...
                            numTimerInterrupts[deviceAddress] = USB_INSERT_TIME;
                            U1OTGIR                 = USB_INTERRUPT_T1MSECIF; // The interrupt is cleared by writing a '1' to the flag.
                            U1OTGIEbits.T1MSECIE    = 1;
                            _USB_SetNextSubSubState(deviceAddress);
                            break;
                ...
                case SUBSTATE_RESET_DEVICE:	// only 1 device it has deviceAddress1.
                    ...
                    switch (usbHostState[deviceAddress] & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SET_RESET:
							...
							/*** move to case SUBSTATE_GET_DESCRIPTOR_SIZE block **************
							...
							**************************************************************/
                            ...
                            U1CONbits.PPBRST                    = 1;
                            U1CONbits.PPBRST                    = 0;
                            //usbDeviceInfo[deviceAddress].flags.bfPingPongIn    = 0;
                            //usbDeviceInfo[deviceAddress].flags.bfPingPongOut   = 0;
                            usbDeviceInfo[1].flags.bfPingPongIn    = 0;
                            usbDeviceInfo[1].flags.bfPingPongOut   = 0;
                            ...
                            U1CONbits.USBRST                    = 1;
                            numTimerInterrupts[deviceAddress]   = USB_RESET_TIME;
                        ...
                        case SUBSUBSTATE_RESET_RECOVERY:
                            DEBUG_PutString( "HOST: Reset recovery.\r\n" );
                            ...
                            numTimerInterrupts[deviceAddress] = USB_RESET_RECOVERY_TIME;
                            U1OTGIR                           = USB_INTERRUPT_T1MSECIF; // The interrupt is cleared by writing a '1' to the flag.
                            U1OTGIEbits.T1MSECIE              = 1;
                ...
                case SUBSTATE_GET_DEVICE_DESCRIPTOR_SIZE:   // all device throu!!
                    ...
                    switch (usbHostState[deviceAddress] & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SEND_GET_DEVICE_DESCRIPTOR_SIZE:
                            ...
                            if (pDeviceDescriptor[deviceAddress] != NULL)
                            {
                                USB_FREE_AND_CLEAR( pDeviceDescriptor[deviceAddress] );
                            }

                            // Prepare a data buffer for us to use.  We'll make it 8 bytes for now,
                            // which is the minimum wMaxPacketSize for EP0.
                            if (pEP0Data[deviceAddress] != NULL)
                            {
                                USB_FREE_AND_CLEAR( pEP0Data[deviceAddress] );
                            }

                            if ((pEP0Data[deviceAddress] = (BYTE *)USB_MALLOC( 8 )) == NULL)
                            {
                                DEBUG_PutString( "HOST: Error alloc-ing pEP0Data\r\n" );

                                _USB_SetErrorCode( deviceAddress, USB_HOLDING_OUT_OF_MEMORY );
                                _USB_SetHoldState( deviceAddress );
                                break;
                            }

                            pEP0Data[deviceAddress][0] = USB_SETUP_DEVICE_TO_HOST | USB_SETUP_TYPE_STANDARD | USB_SETUP_RECIPIENT_DEVICE;
                            pEP0Data[deviceAddress][1] = USB_REQUEST_GET_DESCRIPTOR;
                            pEP0Data[deviceAddress][2] = 0; // Index
                            pEP0Data[deviceAddress][3] = USB_DESCRIPTOR_DEVICE; // Type
                            pEP0Data[deviceAddress][4] = 0;
                            pEP0Data[deviceAddress][5] = 0;
                            pEP0Data[deviceAddress][6] = 8;
                            pEP0Data[deviceAddress][7] = 0;

                            _USB_InitControlRead( usbDeviceInfo[deviceAddress].pEndpoint0, pEP0Data[deviceAddress], 8, pEP0Data[deviceAddress], 8 );
                            _USB_SetNextSubSubState(deviceAddress);
                            break;
                        ...
                        case SUBSUBSTATE_WAIT_FOR_GET_DEVICE_DESCRIPTOR_SIZE:
                            if (usbDeviceInfo[deviceAddress].pEndpoint0->status.bfTransferComplete)
                            {
                                if (usbDeviceInfo[deviceAddress].pEndpoint0->status.bfTransferSuccessful)
                                {
                                    /****
                                    HUB Check Routine move to after Device Addressing. S.Suwa
                                    #ifndef USB_HUB_SUPPORT_INCLUDED
                                        // See if a hub is attached.  Hubs are not supported.
                                        if (pEP0Data[deviceAddress][4] == USB_HUB_CLASSCODE)   // bDeviceClass
                                        {
                                            _USB_SetErrorCode( deviceAddress, USB_HOLDING_UNSUPPORTED_HUB );
                                            _USB_SetHoldState(deviceAddress);
                                        }
                                        else
                                        {
                                ...
                                else
                                {
                                    ...
                                    // enumerate again.
                                    _USB_CheckCommandAndEnumerationAttempts(deviceAddress);
                                }
                            }
                            break;

                        case SUBSUBSTATE_GET_DEVICE_DESCRIPTOR_SIZE_COMPLETE:
                            DEBUG_PutString("\tDevice Descriptor Size=0x"); DEBUG_PutHexUINT8(pEP0Data[deviceAddress][0]);
	                        DEBUG_PutString("\r\n");

                            // Allocate a buffer for the entire Device Descriptor
                            if ((pDeviceDescriptor[deviceAddress] = (BYTE *)USB_MALLOC( pEP0Data[deviceAddress][0] )) == NULL)
                            {
                            ...
                            pDeviceDescriptor[deviceAddress][0] = pEP0Data[deviceAddress][0];

                            // Set the EP0 packet size.
                            usbDeviceInfo[deviceAddress].pEndpoint0->wMaxPacketSize = ((USB_DEVICE_DESCRIPTOR *)(pEP0Data[deviceAddress]))->bMaxPacketSize0;

                            // Make our pEP0Data buffer the size of the max packet.
                            USB_FREE_AND_CLEAR( pEP0Data[deviceAddress] );
                            if ((pEP0Data[deviceAddress] = (BYTE *)USB_MALLOC( usbDeviceInfo[deviceAddress].pEndpoint0->wMaxPacketSize )) == NULL)
                            {
                                // We cannot continue.  Freeze until the device is removed.
                ...
                case SUBSTATE_GET_DEVICE_DESCRIPTOR:
                    ...
                    switch (usbHostState[deviceAddress] & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SEND_GET_DEVICE_DESCRIPTOR:
                            ...
                            pEP0Data[deviceAddress][4] = 0;
                            pEP0Data[deviceAddress][5] = 0;
                            pEP0Data[deviceAddress][6] = pDeviceDescriptor[deviceAddress][0];
                            pEP0Data[deviceAddress][7] = 0;
                            _USB_InitControlRead( usbDeviceInfo[deviceAddress].pEndpoint0, pEP0Data[deviceAddress], 8, pDeviceDescriptor[deviceAddress], pDeviceDescriptor[deviceAddress][0]  );
                            _USB_SetNextSubSubState(deviceAddress);
                            break;
                        ...
                        case SUBSUBSTATE_GET_DEVICE_DESCRIPTOR_COMPLETE:
	                        DEBUG_PutString("HOST: Getting DEVICE DESCRIPTOR complete.\r\n" );
                            // Clean up and advance to the next substate.
                            _USB_InitErrorCounters(deviceAddress);
                            _USB_SetNextSubState(deviceAddress);
                            break;
                ...
                case SUBSTATE_VALIDATE_VID_PID:
                    ...
                    _USB_FindDeviceLevelClientDriver(deviceAddress);
                ...
                case SUBSTATE_SET_DEVICE_ADDRESS:
                    ...
                    switch (usbHostState[deviceAddress] & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SEND_SET_DEVICE_ADDRESS:
    						...
    						DEBUG_PutString("\taddress=0x"); DEBUG_PutHexUINT8(deviceAddress); DEBUG_PutString("\r\n");
                            ...
                            usbDeviceInfo[deviceAddress].deviceAddress = deviceAddress;
							

                            // Set up and send SET ADDRESS
                            pEP0Data[deviceAddress][0] = USB_SETUP_HOST_TO_DEVICE | USB_SETUP_TYPE_STANDARD | USB_SETUP_RECIPIENT_DEVICE;
                            pEP0Data[deviceAddress][1] = USB_REQUEST_SET_ADDRESS;
                            pEP0Data[deviceAddress][2] = usbDeviceInfo[deviceAddress].deviceAddress;
                            pEP0Data[deviceAddress][3] = 0;
                            pEP0Data[deviceAddress][4] = 0;
                            pEP0Data[deviceAddress][5] = 0;
                            pEP0Data[deviceAddress][6] = 0;
                            pEP0Data[deviceAddress][7] = 0;
                            _USB_InitControlWrite( usbDeviceInfo[deviceAddress].pEndpoint0, pEP0Data[deviceAddress], 8, NULL, 0 );
                            _USB_SetNextSubSubState(deviceAddress);
                            break;
                        ...
                        case SUBSUBSTATE_SET_DEVICE_ADDRESS_COMPLETE:
                            ...
                            usbDeviceInfo[deviceAddress].deviceAddressAndSpeed = (usbDeviceInfo[deviceAddress].flags.bfIsLowSpeed << 7) | usbDeviceInfo[deviceAddress].deviceAddress;

							// check HUB Device in here. after device addressing.
                            if (pDeviceDescriptor[deviceAddress][4] == USB_HUB_CLASSCODE)   // bDeviceClass
                            {
		                        /*** cut here, because we support a cascade HUB
							    if(deviceAddress > 1) // cascade HUB ?
                                {
                                    // Cascade Hub are not supported.
                                    _USB_SetErrorCode( deviceAddress, USB_HOLDING_UNSUPPORTED_HUB );
                                    _USB_SetHoldState(deviceAddress);
                                    break;
                                }
        ...
        case STATE_CONFIGURING:
            switch (usbHostState[deviceAddress] & SUBSTATE_MASK)
            {
                case SUBSTATE_INIT_CONFIGURATION:
                    ...
                    // the list.
                    countConfigurations[deviceAddress] = ((USB_DEVICE_DESCRIPTOR *)pDeviceDescriptor[deviceAddress])->bNumConfigurations;
                    while (usbDeviceInfo[deviceAddress].pConfigurationDescriptorList != NULL)
                    {
                        pTemp = (BYTE *)usbDeviceInfo[deviceAddress].pConfigurationDescriptorList->next;
                        USB_FREE_AND_CLEAR( usbDeviceInfo[deviceAddress].pConfigurationDescriptorList->descriptor );
                        USB_FREE_AND_CLEAR( usbDeviceInfo[deviceAddress].pConfigurationDescriptorList );
                        usbDeviceInfo[deviceAddress].pConfigurationDescriptorList = (USB_CONFIGURATION *)pTemp;
                    }
                    _USB_SetNextSubState(deviceAddress);
                    break;
                ...
                case SUBSTATE_GET_CONFIG_DESCRIPTOR_SIZE:
                    ...
                    switch (usbHostState[deviceAddress] & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SEND_GET_CONFIG_DESCRIPTOR_SIZE:
                            ...
                            pEP0Data[deviceAddress][2] = 0;
                            //pEP0Data[2] = countConfigurations-1;    // USB 2.0 - range is 0 - count-1
                            pEP0Data[deviceAddress][3] = USB_DESCRIPTOR_CONFIGURATION;
                            pEP0Data[deviceAddress][4] = 0;
                            pEP0Data[deviceAddress][5] = 0;
                            pEP0Data[deviceAddress][6] = 8;
                            pEP0Data[deviceAddress][7] = 0;
                        ...
                        case SUBSUBSTATE_GET_CONFIG_DESCRIPTOR_SIZECOMPLETE:
                            ...
                            if ((((USB_CONFIGURATION *)pTemp)->descriptor = (BYTE *)USB_MALLOC( ((WORD)(pEP0Data[deviceAddress][3]) << 8) + (WORD)(pEP0Data[deviceAddress][2]) )) == NULL)
                            {
                            ...
                            ((USB_CONFIGURATION_DESCRIPTOR *)((USB_CONFIGURATION *)pTemp)->descriptor)->wTotalLength =
                                    ((WORD)(pEP0Data[deviceAddress][3]) << 8) + (WORD)(pEP0Data[deviceAddress][2]);

                            // Put the new node at the front of the list.
                            ((USB_CONFIGURATION *)pTemp)->next = usbDeviceInfo[deviceAddress].pConfigurationDescriptorList;
                            usbDeviceInfo[deviceAddress].pConfigurationDescriptorList = (USB_CONFIGURATION *)pTemp;

                            // Save the configuration descriptor pointer and number
                            pCurrentConfigurationDescriptor[deviceAddress] = ((USB_CONFIGURATION *)pTemp)->descriptor;
                            ((USB_CONFIGURATION *)pTemp)->configNumber = countConfigurations[deviceAddress];
                ...
                case SUBSTATE_GET_CONFIG_DESCRIPTOR:
                    ...
                    switch (usbHostState[deviceAddress] & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SEND_GET_CONFIG_DESCRIPTOR:
                            ...
                            pEP0Data[deviceAddress][4] = 0;
                            pEP0Data[deviceAddress][5] = 0;
                            pEP0Data[deviceAddress][6] = usbDeviceInfo[deviceAddress].pConfigurationDescriptorList->descriptor[2];    // wTotalLength
                            pEP0Data[deviceAddress][7] = usbDeviceInfo[deviceAddress].pConfigurationDescriptorList->descriptor[3];
                            _USB_InitControlRead( usbDeviceInfo[deviceAddress].pEndpoint0, pEP0Data[deviceAddress], 8, usbDeviceInfo[deviceAddress].pConfigurationDescriptorList->descriptor,
                                    ((USB_CONFIGURATION_DESCRIPTOR *)usbDeviceInfo[deviceAddress].pConfigurationDescriptorList->descriptor)->wTotalLength );
                        ...
                        case SUBSUBSTATE_GET_CONFIG_DESCRIPTOR_COMPLETE:
	                        DEBUG_PutString( "HOST: Getting configuration device descriptor complete.\r\n" );
                            // Clean up and advance to the next state.  Keep the data for later use.
                            _USB_InitErrorCounters(deviceAddress);
                            countConfigurations[deviceAddress] --;
                            if (countConfigurations[deviceAddress])
                            {
                                // There are more descriptors that we need to get.
                                usbHostState[deviceAddress] = STATE_CONFIGURING | SUBSTATE_GET_CONFIG_DESCRIPTOR_SIZE;
                            }
                            else
                            {
                ...
                case SUBSTATE_SELECT_CONFIGURATION:
                    ...
                    switch (usbHostState[deviceAddress] & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SELECT_CONFIGURATION:
                            ...
                            _USB_FreeConfigMemory(deviceAddress);

                            // If the configuration wasn't selected based on the VID & PID
                            if (usbDeviceInfo[deviceAddress].currentConfiguration == 0)
                            {
                                // Search for a supported class-specific configuration.
                                pCurrentConfigurationNode = usbDeviceInfo[deviceAddress].pConfigurationDescriptorList;
                                while (pCurrentConfigurationNode)
                                {
                                    pCurrentConfigurationDescriptor[deviceAddress] = pCurrentConfigurationNode->descriptor;
                                    if (_USB_ParseConfigurationDescriptor(deviceAddress))
                                    {
                                        break;
                                    }
                                    else
                            ...
                            else
                            {
                                ...
                                pCurrentConfigurationNode = usbDeviceInfo[deviceAddress].pConfigurationDescriptorList;
                                while (pCurrentConfigurationNode && pCurrentConfigurationNode->configNumber != usbDeviceInfo[deviceAddress].currentConfiguration)
                                {
                                    pCurrentConfigurationNode = pCurrentConfigurationNode->next;
                                }
                                pCurrentConfigurationDescriptor[deviceAddress] = pCurrentConfigurationNode->descriptor;
                            ...
                            //If No OTG Then
                            if (usbDeviceInfo[deviceAddress].flags.bfConfiguredOTG)
                            {
                                // Did we fail to configure?
                                if (pCurrentConfigurationNode == NULL)
                                {
                                    // Failed to find a supported configuration.
                                    _USB_SetErrorCode( deviceAddress, USB_HOLDING_UNSUPPORTED_DEVICE );
                                    _USB_SetHoldState(deviceAddress);
                                }
                                else
                                {
                        ...
                        case SUBSUBSTATE_SEND_SET_OTG:
                            ...
                            if (!usbDeviceInfo[deviceAddress].flags.bfConfiguredOTG)
                            {
                                ...
                                usbDeviceInfo[deviceAddress].flags.bfConfiguredOTG = 1;

                                // Send SET FEATURE
                                pEP0Data[deviceAddress][0] = USB_SETUP_HOST_TO_DEVICE | USB_SETUP_TYPE_STANDARD | USB_SETUP_RECIPIENT_DEVICE;
                                pEP0Data[deviceAddress][1] = USB_REQUEST_SET_FEATURE;
                                if (usbDeviceInfo[deviceAddress].flags.bfAllowHNP) // Needs to be set by the user
                                {
                                    pEP0Data[deviceAddress][2] = OTG_FEATURE_B_HNP_ENABLE;
                                }
                                else
                                {
                                    pEP0Data[deviceAddress][2] = OTG_FEATURE_A_HNP_SUPPORT;
                                }
                                pEP0Data[deviceAddress][3] = 0;
                                pEP0Data[deviceAddress][4] = 0;
                                pEP0Data[deviceAddress][5] = 0;
                        ...
                        case SUBSUBSTATE_WAIT_FOR_SET_OTG_DONE:
                            if (usbDeviceInfo[deviceAddress].pEndpoint0->status.bfTransferComplete)
                            {
                                ...
                                {
                                    #ifdef  USB_SUPPORT_OTG
                                        if (usbDeviceInfo[deviceAddress].flags.bfAllowHNP)
                                        {
                                            USBOTGEnableHnp();
                                        }
                                     #endif
                ...
                case SUBSTATE_SET_CONFIGURATION:
                    ...
                    switch (usbHostState[deviceAddress] & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SEND_SET_CONFIGURATION:
                            ...
                            DEBUG_PutString("\tcurrent Configuration is 0x");
                            DEBUG_PutHexUINT8(usbDeviceInfo[deviceAddress].currentConfiguration);
                            DEBUG_PutString("\r\n");

                            // Set up and send SET CONFIGURATION.
                            pEP0Data[deviceAddress][0] = USB_SETUP_HOST_TO_DEVICE | USB_SETUP_TYPE_STANDARD | USB_SETUP_RECIPIENT_DEVICE;
                            pEP0Data[deviceAddress][1] = USB_REQUEST_SET_CONFIGURATION;
                            pEP0Data[deviceAddress][2] = usbDeviceInfo[deviceAddress].currentConfiguration;
                            pEP0Data[deviceAddress][3] = 0;
                            pEP0Data[deviceAddress][4] = 0;
                            pEP0Data[deviceAddress][5] = 0;
                            pEP0Data[deviceAddress][6] = 0;
                        ...
                        case SUBSUBSTATE_INIT_CLIENT_DRIVERS:
                            ...
                            // if connecting device is Hub, then Hub initialize.
                            ...
                            if (usbDeviceInfo[deviceAddress].flags.bfUseDeviceClientDriver)
                            {
                                ...
                                if (!usbClientDrvTable[temp].Initialize(usbDeviceInfo[deviceAddress].deviceAddress, usbClientDrvTable[temp].flags, temp))
                                {
                                    _USB_SetErrorCode( deviceAddress, USB_HOLDING_CLIENT_INIT_ERROR );
                                    _USB_SetHoldState(deviceAddress);
                                }
                            ...
                            else
                            {
                                ...
                                pCurrentInterface = usbDeviceInfo[deviceAddress].pInterfaceList;
                            ...
                            if(usbDeviceInfo[deviceAddress].flags.bfUseEP0Driver == 1)
                            {
                                temp = usbDeviceInfo[deviceAddress].deviceEP0Driver;
                                if (!usbClientDrvTable[temp].Initialize(usbDeviceInfo[deviceAddress].deviceAddress, usbClientDrvTable[temp].flags, temp))
                                {
                                    _USB_SetErrorCode( deviceAddress, USB_HOLDING_CLIENT_INIT_ERROR );
                                    _USB_SetHoldState(deviceAddress);
                ...
                case SUBSTATE_SUSPEND_AND_RESUME:
                    switch (usbHostState[deviceAddress] & SUBSUBSTATE_MASK)
                    {
                        ...
                        case SUBSUBSTATE_RESUME:
                            ...
                            numTimerInterrupts[deviceAddress]      = USB_RESUME_TIME;
                            U1OTGIR                 = USB_INTERRUPT_T1MSECIF; // The interrupt is cleared by writing a '1' to the flag.
                            U1OTGIEbits.T1MSECIE    = 1;
                        ...
                        case SUBSUBSTATE_RESUME_RECOVERY:
                            ...
                            numTimerInterrupts[deviceAddress]      = USB_RESUME_RECOVERY_TIME;
                            U1OTGIR                 = USB_INTERRUPT_T1MSECIF; // The interrupt is cleared by writing a '1' to the flag.
                            U1OTGIEbits.T1MSECIE    = 1;
                        ...
                        case SUBSUBSTATE_RESUME_COMPLETE:
                            ...
                            usbHostState[deviceAddress] = STATE_RUNNING | SUBSTATE_NORMAL_RUN;
                            break;
                    }
            }
            break;
        ...
        case STATE_HOLDING:
            switch (usbHostState[deviceAddress] & SUBSTATE_MASK)
            {
                ...
                if(deviceAddress == 1){
                    ...
                    switch (usbDeviceInfo[deviceAddress].errorCode )
                    {
                    ...
                    USB_HOST_APP_EVENT_HANDLER( usbDeviceInfo[deviceAddress].deviceAddress, temp, &usbDeviceInfo[deviceAddress].currentConfigurationPower , 1 );
...
//	DEBUG_PutString("HOST: USBHostTerminateTransfer(deviceAddress="); DEBUG_PutDec(deviceAddress);	
//	DEBUG_PutString(", endpoint=0x"); DEBUG_PutHexUINT8(endpoint); DEBUG_PutString(")\r\n");
...
/*	DEBUG_PutString("HOST: USBHostTransferIsComplete(deviceAddress=0"); DEBUG_PutDec(deviceAddress);	
	DEBUG_PutString(", endpoint=0x"); DEBUG_PutHexUINT8(endpoint);
	DEBUG_PutString(", *errorCode)\r\n");
*/
    // Find the required device
    if (deviceAddress != usbDeviceInfo[deviceAddress].deviceAddress)
    {
...
{
/*	DEBUG_PutString("HOST: USBHostVbusEvent(vbusEvent=0x"); DEBUG_PutHexUINT8(vbusEvent);
	DEBUG_PutString(", hubAddress=0x"); DEBUG_PutHexUINT8(hubAddress);	
	DEBUG_PutString(", portNumber=0x"); DEBUG_PutHexUINT8(portNumber);
	DEBUG_PutString(")\r\n");
*/
    BYTE dev;
        ...
        (portNumber == 0 ))
    {
        ...
        {
            //USBHostShutdown(0);
            for(dev=0;dev<USB_MAX_ADDRESS+1;dev++)USBHostShutdown(dev); // All device Shutdown S.Suwa fix.2013/11/16
            usbRootHubInfo.flags.bPowerGoodPort0 = 0;
            return USB_SUCCESS;
        }
...
/*	DEBUG_PutString("HOST: USBHostWrite(deviceAddress="); DEBUG_PutDec(deviceAddress);
	DEBUG_PutString(", endpoint=0x"); DEBUG_PutHexUINT8(endpoint);	
	DEBUG_PutString(", *data, size=0x"); DEBUG_PutHexUINT32(size); DEBUG_PutString(")\r\n");
*/
    // Find the required device
    if (deviceAddress != usbDeviceInfo[deviceAddress].deviceAddress)
    {
...
// change MACRO to function  S.Suwa

void _USB_InitErrorCounters( BYTE deviceAddress )
{
    numCommandTries[deviceAddress] = USB_NUM_COMMAND_TRIES;
}


void _USB_SetDATA01(  BYTE deviceAddress, BYTE DataPID )
{
    pCurrentEndpoint[deviceAddress]->status.bfNextDATA01 = DataPID;
}


void _USB_SetErrorCode( BYTE deviceAddress, BYTE errorCode )
{
	usbDeviceInfo[deviceAddress].errorCode = errorCode;
}


void _USB_SetHoldState( BYTE deviceAddress )
{
	usbHostState[deviceAddress] = STATE_HOLDING;
}


void  _USB_SetNextState( BYTE deviceAddress )
{
	usbHostState[deviceAddress] = (usbHostState[deviceAddress] & STATE_MASK) + NEXT_STATE;
}


void  _USB_SetNextSubState( BYTE deviceAddress )
{
	usbHostState[deviceAddress] = (usbHostState[deviceAddress] & (STATE_MASK | SUBSTATE_MASK)) + NEXT_SUBSTATE;
}


void  _USB_SetNextSubSubState( BYTE deviceAddress )
{
	usbHostState[deviceAddress] +=  NEXT_SUBSUBSTATE;
}


void  _USB_SetPreviousSubSubState( BYTE deviceAddress )
{
	usbHostState[deviceAddress] -= NEXT_SUBSUBSTATE;
}
...
void _USB_CheckCommandAndEnumerationAttempts( BYTE deviceAddress )
{
//  DEBUG_PutChar( '=' );

    // Clear the error and stall flags.  A stall here does not require
    // host intervention to clear.
    pCurrentEndpoint[deviceAddress]->status.bfError    = 0;
    pCurrentEndpoint[deviceAddress]->status.bfStalled  = 0;

    numCommandTries[deviceAddress] --;
    if (numCommandTries[deviceAddress] != 0)
    {
        // We still have retries left on this command.  Try again.
        usbHostState[deviceAddress] &= ~SUBSUBSTATE_MASK;
    }
    else
    {
        // This command has timed out.
        // We are enumerating.  See if we can try to enumerate again.
        numEnumerationTries[deviceAddress] --;
        if (numEnumerationTries[deviceAddress] != 0)
        {
            // We still have retries left to try to enumerate.  Reset and try again.
            usbHostState[deviceAddress] = STATE_ATTACHED | SUBSTATE_RESET_DEVICE;
        }
        else
        {
            // Give up.  The device is not responding properly.
            _USB_SetErrorCode( deviceAddress, USB_CANNOT_ENUMERATE );
            _USB_SetHoldState(deviceAddress);
        }
    }
}
...
BOOL _USB_FindClassDriver( BYTE deviceAddress, BYTE bClass, BYTE bSubClass, BYTE bProtocol, BYTE *pbClientDrv ) // 2014/1/21 S.Suwa
{
    USB_OVERRIDE_CLIENT_DRIVER_EVENT_DATA   eventData;
    int                                     i;
    USB_DEVICE_DESCRIPTOR                   *pDesc = (USB_DEVICE_DESCRIPTOR *)pDeviceDescriptor[deviceAddress]; // 2014/1/21 S.Suwa

	DEBUG_PutString("HOST: _USB_FindClassDriver(deviceAddress=0x"); DEBUG_PutHexUINT8(deviceAddress);
        DEBUG_PutString(", bClass=0x"); DEBUG_PutHexUINT8(bClass);
	DEBUG_PutString(", bSubClass=0x"); DEBUG_PutHexUINT8(bSubClass);	
	DEBUG_PutString(", bProtocol=0x"); DEBUG_PutHexUINT8(bProtocol);
	DEBUG_PutString(", *pbClientDrv)\r\n");
...
BOOL _USB_FindDeviceLevelClientDriver( BYTE deviceAddress )
{
    WORD                   i;
    USB_DEVICE_DESCRIPTOR *pDesc = (USB_DEVICE_DESCRIPTOR *)pDeviceDescriptor[deviceAddress]; // 2014/1/21 S.Suwa

	DEBUG_PutString("HOST: _USB_FindDeviceLevelClientDriver(deviceAddress="); DEBUG_PutDec(deviceAddress); DEBUG_PutString(")\r\n");

    // Scan TPL
    i = 0;
    usbDeviceInfo[deviceAddress].flags.bfUseDeviceClientDriver = 0;
    usbDeviceInfo[deviceAddress].flags.bfUseEP0Driver = 0;
    while (i < NUM_TPL_ENTRIES)
    {
        if (usbTPL[i].flags.bfIsClassDriver)
        {
                ...
                (usbTPL[i].device.bProtocol == pDesc->bDeviceProtocol)   )
            {
                ...
                usbDeviceInfo[deviceAddress].flags.bfUseDeviceClientDriver = 1;
            }
        }
        else
        {
                ...
                (usbTPL[i].device.idProduct == pDesc->idProduct))
            {
                ...
                {
                    usbDeviceInfo[deviceAddress].flags.bfUseEP0Driver = 1;
                    usbDeviceInfo[deviceAddress].deviceEP0Driver = usbTPL[i].ClientDriver;

                    // Select configuration if it is given in the TPL
                    if (usbTPL[i].flags.bfSetConfiguration)
                    {
                        usbDeviceInfo[deviceAddress].currentConfiguration = usbTPL[i].bConfiguration;
                    }
                }
                else
                {
        ...
        if (usbDeviceInfo[deviceAddress].flags.bfUseDeviceClientDriver)
        {
            ...
            usbDeviceInfo[deviceAddress].deviceClientDriver = usbTPL[i].ClientDriver;
...
USB_ENDPOINT_INFO * _USB_FindEndpoint( BYTE deviceAddress, BYTE endpoint )
{
    USB_ENDPOINT_INFO           *pEndpoint;
    USB_INTERFACE_INFO          *pInterface;

//  DEBUG_PutString("HOST: _USB_FindEndpoint(deviceAddress="); DEBUG_PutDec(deviceAddress);
//  DEBUG_PutString(", endpoint=0x"); DEBUG_PutHexUINT8(endpoint); DEBUG_PutString(")\r\n");

    if (endpoint == 0)
    {
        return usbDeviceInfo[deviceAddress].pEndpoint0;
    }
...
/*  
// 
USB_INTERFACE_INFO * _USB_FindInterface ( BYTE deviceAddress, BYTE bInterface, BYTE bAltSetting )
{
    USB_INTERFACE_INFO *pCurIntf = usbDeviceInfo[deviceAddress].pInterfaceList;
...
#if 0
void _USB_FindNextToken( BYTE deviceAddress )
{
    BOOL    illegalState = FALSE;

//	DEBUG_PutString("HOST: _USB_FindNextToken(deviceAddress="); DEBUG_PutDec(deviceAddress); DEBUG_PutString(")\r\n");
    ...
    // has expired.
    if ((usbHostState[deviceAddress] & (SUBSTATE_MASK | SUBSUBSTATE_MASK)) == (STATE_RUNNING | SUBSTATE_SUSPEND_AND_RESUME))
    {
        return;
    }
    ...
    if (!usbBusInfo.flags.bfControlTransfersDone)
    {
        ...
        if (_USB_FindServiceEndpoint( deviceAddress, USB_TRANSFER_TYPE_CONTROL ))
        {
            switch (pCurrentEndpoint[deviceAddress]->transferState & TSTATE_MASK)
            {
                case TSTATE_CONTROL_NO_DATA:
                    switch (pCurrentEndpoint[deviceAddress]->transferState & TSUBSTATE_MASK)
                    {
                        case TSUBSTATE_CONTROL_NO_DATA_SETUP:
                            _USB_SetDATA01( deviceAddress, DTS_DATA0 );
                            _USB_SetBDT( deviceAddress, USB_TOKEN_SETUP );
                            _USB_SendToken( deviceAddress, pCurrentEndpoint[deviceAddress]->bEndpointAddress, USB_TOKEN_SETUP );
                        ...
                        case TSUBSTATE_CONTROL_NO_DATA_ACK:
                            pCurrentEndpoint[deviceAddress]->dataCountMax = pCurrentEndpoint[deviceAddress]->dataCount;
                            _USB_SetDATA01( deviceAddress, DTS_DATA1 );
                            _USB_SetBDT( deviceAddress, USB_TOKEN_IN );
                            _USB_SendToken( deviceAddress, pCurrentEndpoint[deviceAddress]->bEndpointAddress, USB_TOKEN_IN );
                            #ifdef ONE_CONTROL_TRANSACTION_PER_FRAME
                                usbBusInfo.flags.bfControlTransfersDone = 1; // Only one control transfer per frame.
                            #endif
                            return;
                        ...
                        case TSUBSTATE_CONTROL_NO_DATA_COMPLETE:
                            pCurrentEndpoint[deviceAddress]->transferState               = TSTATE_IDLE;
                            pCurrentEndpoint[deviceAddress]->status.bfTransferComplete   = 1;
                            #if defined( USB_ENABLE_TRANSFER_EVENT )
                                if (StructQueueIsNotFull(&usbEventQueue, USB_EVENT_QUEUE_DEPTH))
                                {
                                    ...
                                    data->TransferData.dataCount        = pCurrentEndpoint[deviceAddress]->dataCount;
                                    data->TransferData.pUserData        = pCurrentEndpoint[deviceAddress]->pUserData;
                                    data->TransferData.bErrorCode       = USB_SUCCESS;
                                    data->TransferData.bEndpointAddress = pCurrentEndpoint[deviceAddress]->bEndpointAddress;
                                    data->TransferData.bmAttributes.val = pCurrentEndpoint[deviceAddress]->bmAttributes.val;
                                    data->TransferData.clientDriver     = pCurrentEndpoint[deviceAddress]->clientDriver;
									data->address = deviceAddress;
                                }
                                else
                                {
                                    pCurrentEndpoint[deviceAddress]->bmAttributes.val = USB_EVENT_QUEUE_FULL;
                                }
                            #endif
                        break;
                        ...
                        case TSUBSTATE_ERROR:
                            ...
                            pCurrentEndpoint[deviceAddress]->status.bfTransferComplete   = 1;
                                ...
                                if (StructQueueIsNotFull(&usbEventQueue, USB_EVENT_QUEUE_DEPTH))
                                {
                                    ...
                                    data->TransferData.bErrorCode       = pCurrentEndpoint[deviceAddress]->bErrorCode;
                                    data->TransferData.bEndpointAddress = pCurrentEndpoint[deviceAddress]->bEndpointAddress;
                                    data->TransferData.bmAttributes.val = pCurrentEndpoint[deviceAddress]->bmAttributes.val;
                                    data->TransferData.clientDriver     = pCurrentEndpoint[deviceAddress]->clientDriver;
									data->address = deviceAddress;
                ...
                case TSTATE_CONTROL_READ:
                    switch (pCurrentEndpoint[deviceAddress]->transferState & TSUBSTATE_MASK)
                    {
                        ...
                        case TSUBSTATE_CONTROL_READ_ACK:
                            pCurrentEndpoint[deviceAddress]->dataCountMax = pCurrentEndpoint[deviceAddress]->dataCount;
                            _USB_SetDATA01( deviceAddress, DTS_DATA1 );
                            _USB_SetBDT( deviceAddress, USB_TOKEN_OUT );
                            _USB_SendToken( deviceAddress, pCurrentEndpoint[deviceAddress]->bEndpointAddress, USB_TOKEN_OUT );
                            #ifdef ONE_CONTROL_TRANSACTION_PER_FRAME
                                usbBusInfo.flags.bfControlTransfersDone = 1; // Only one control transfer per frame.
                            #endif
                            return;
        ...
        if (!usbBusInfo.flags.bfIsochronousTransfersDone)
        {
            ...
            //while (_USB_FindServiceEndpoint( deviceAddress, USB_TRANSFER_TYPE_ISOCHRONOUS ))
            if(_USB_FindServiceEndpoint( deviceAddress, USB_TRANSFER_TYPE_ISOCHRONOUS ))
            {
                switch (pCurrentEndpoint[deviceAddress]->transferState & TSTATE_MASK)
                {
                    case TSTATE_ISOCHRONOUS_READ:
                        switch (pCurrentEndpoint[deviceAddress]->transferState & TSUBSTATE_MASK)
                        {
                            case TSUBSTATE_ISOCHRONOUS_READ_DATA:
                                ...
                                if (((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->buffers[((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->currentBufferUSB].bfDataLengthValid)
                                {
                                ...
                                else
                                {
                                    ...
                                    ((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->buffers[((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->currentBufferUSB].bfDataLengthValid = 0;
                                    pCurrentEndpoint[deviceAddress]->dataCount = 0;

                                    _USB_SetDATA01( deviceAddress, DTS_DATA0 );    // Always DATA0 for isochronous
                                    _USB_SetBDT( deviceAddress, USB_TOKEN_IN );
                                    _USB_SendToken( deviceAddress, pCurrentEndpoint[deviceAddress]->bEndpointAddress, USB_TOKEN_IN );
                                    return;
                                }    
                            ...
                            case TSUBSTATE_ISOCHRONOUS_READ_COMPLETE:
                                ...
                                // interval.
                                pCurrentEndpoint[deviceAddress]->transferState     = TSTATE_ISOCHRONOUS_READ | TSUBSTATE_ISOCHRONOUS_READ_DATA;
                                pCurrentEndpoint[deviceAddress]->wIntervalCount    = pCurrentEndpoint[deviceAddress]->wInterval;

                                // Update the valid data length for this buffer.
                                ((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->buffers[((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->currentBufferUSB].dataLength = pCurrentEndpoint[deviceAddress]->dataCount;
                                ((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->buffers[((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->currentBufferUSB].bfDataLengthValid = 1;
                                ...
                                // possible, send up the event.  Then mark the packet as used.
                                    ...
                                    usbClientDrvTable[pCurrentEndpoint[deviceAddress]->clientDriver].DataEventHandler( usbDeviceInfo[deviceAddress].deviceAddress, EVENT_DATA_ISOC_READ, ((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->buffers[((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->currentBufferUSB].pBuffer, pCurrentEndpoint[deviceAddress]->dataCount );
                                    ((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->buffers[((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->currentBufferUSB].bfDataLengthValid = 0;
                                #endif
                                
                                // Move to the next data buffer.
                                ((ISOCHRONOUS_DATA *)pCurrentEndpoint[deviceAddress]->pUserData)->currentBufferUSB++;
                                if (((ISOCHRONOUS_DATA *)pCurrentEndpoint[deviceAddress]->pUserData)->currentBufferUSB >= ((ISOCHRONOUS_DATA *)pCurrentEndpoint[deviceAddress]->pUserData)->totalBuffers)
                                {
                                    ((ISOCHRONOUS_DATA *)pCurrentEndpoint[deviceAddress]->pUserData)->currentBufferUSB = 0;
                                }
                                break;
                    ...
                    case TSTATE_ISOCHRONOUS_WRITE:
                        switch (pCurrentEndpoint[deviceAddress]->transferState & TSUBSTATE_MASK)
                        {
                            case TSUBSTATE_ISOCHRONOUS_WRITE_DATA:
                                ...
                                {
                                    pCurrentEndpoint[deviceAddress]->dataCount = ((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->buffers[((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->currentBufferUSB].dataLength;
                            ...
                            case TSUBSTATE_ISOCHRONOUS_WRITE_COMPLETE:
                                ...
                                // interval.
                                pCurrentEndpoint[deviceAddress]->wIntervalCount    = pCurrentEndpoint[deviceAddress]->wInterval;
                                pCurrentEndpoint[deviceAddress]->transferState     = TSTATE_ISOCHRONOUS_WRITE | TSUBSTATE_ISOCHRONOUS_WRITE_DATA;
                                ...
                                ((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->buffers[((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->currentBufferUSB].bfDataLengthValid = 0;
                                    ...
                                    if (StructQueueIsNotFull(&usbEventQueue, USB_EVENT_QUEUE_DEPTH))
                                    {
                                        ...
                                        data->event = EVENT_TRANSFER;
                                        data->TransferData.dataCount        = pCurrentEndpoint[deviceAddress]->dataCount;
                                        data->TransferData.pUserData        = ((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->buffers[((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->currentBufferUSB].pBuffer;
                                        data->TransferData.bErrorCode       = USB_SUCCESS;
                                        data->TransferData.bEndpointAddress = pCurrentEndpoint[deviceAddress]->bEndpointAddress;
                                        data->TransferData.bmAttributes.val = pCurrentEndpoint[deviceAddress]->bmAttributes.val;
                                        data->TransferData.clientDriver     = pCurrentEndpoint[deviceAddress]->clientDriver;
                                ...
                                #ifdef USB_HOST_APP_DATA_EVENT_HANDLER
                                    usbClientDrvTable[pCurrentEndpoint[deviceAddress]->clientDriver].DataEventHandler( usbDeviceInfo[deviceAddress].deviceAddress, EVENT_DATA_ISOC_WRITE, ((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->buffers[((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->currentBufferUSB].pBuffer, pCurrentEndpoint[deviceAddress]->dataCount );
                                #endif
        ...
        if (!usbBusInfo.flags.bfInterruptTransfersDone)
        {
            ...
            while (_USB_FindServiceEndpoint( deviceAddress, USB_TRANSFER_TYPE_INTERRUPT ))
            {
                switch (pCurrentEndpoint[deviceAddress]->transferState & TSTATE_MASK)
                {
...
TryBulk:
        ...
        if (!usbBusInfo.flags.bfBulkTransfersDone)
        {
            ...
            if (_USB_FindServiceEndpoint( deviceAddress, USB_TRANSFER_TYPE_BULK ))
            {
                switch (pCurrentEndpoint[deviceAddress]->transferState & TSTATE_MASK)
                {
...
BOOL _USB_FindServiceEndpoint( BYTE deviceAddress, BYTE transferType )
{
    USB_ENDPOINT_INFO           *pEndpoint;
    USB_INTERFACE_INFO          *pInterface;

//	DEBUG_PutString("HOST: _USB_FindServiceEndpoint(deviceAddress="); DEBUG_PutDec(deviceAddress);
//  DEBUG_PutString(", transferType=0x"); DEBUG_PutHexUINT8(transferType); DEBUG_PutString(")\r\n");

    // Check endpoint 0.
    if ((usbDeviceInfo[deviceAddress].pEndpoint0->bmAttributes.bfTransferType == transferType) &&
        !usbDeviceInfo[deviceAddress].pEndpoint0->status.bfTransferComplete)
    {
        pCurrentEndpoint[deviceAddress] = usbDeviceInfo[deviceAddress].pEndpoint0;
        return TRUE;
    }
			...
			if (pEndpoint->bmAttributes.bfTransferType == transferType)
			{
				switch (transferType)
				{
					case USB_TRANSFER_TYPE_CONTROL:
						if (!pEndpoint->status.bfTransferComplete)
						{
							pCurrentEndpoint[deviceAddress] = pEndpoint;
							return TRUE;
						}
						break;
...
void _USB_FreeConfigMemory( BYTE deviceAddress )
{
    USB_INTERFACE_INFO          *pTempInterface;
    USB_INTERFACE_SETTING_INFO  *pTempSetting;
    USB_ENDPOINT_INFO           *pTempEndpoint;

    while (usbDeviceInfo[deviceAddress].pInterfaceList != NULL)
    {
        pTempInterface = usbDeviceInfo[deviceAddress].pInterfaceList->next;

        while (usbDeviceInfo[deviceAddress].pInterfaceList->pInterfaceSettings != NULL)
        {
            pTempSetting = usbDeviceInfo[deviceAddress].pInterfaceList->pInterfaceSettings->next;

            while (usbDeviceInfo[deviceAddress].pInterfaceList->pInterfaceSettings->pEndpointList != NULL)
            {
                pTempEndpoint = usbDeviceInfo[deviceAddress].pInterfaceList->pInterfaceSettings->pEndpointList->next;
                USB_FREE_AND_CLEAR( usbDeviceInfo[deviceAddress].pInterfaceList->pInterfaceSettings->pEndpointList );
                usbDeviceInfo[deviceAddress].pInterfaceList->pInterfaceSettings->pEndpointList = pTempEndpoint;
            }
            USB_FREE_AND_CLEAR( usbDeviceInfo[deviceAddress].pInterfaceList->pInterfaceSettings );
            usbDeviceInfo[deviceAddress].pInterfaceList->pInterfaceSettings = pTempSetting;
        }
        USB_FREE_AND_CLEAR( usbDeviceInfo[deviceAddress].pInterfaceList );
        usbDeviceInfo[deviceAddress].pInterfaceList = pTempInterface;
    }
...
/****************************************************************************
  ...
  ***************************************************************************/
// 
void _USB_FreeMemory( BYTE deviceAddress )
{
    BYTE    *pTemp;
    ...
    if (pDeviceDescriptor[deviceAddress] != NULL)   // fix. 2013/11/09 Suwa
    {
        USB_FREE_AND_CLEAR( pDeviceDescriptor[deviceAddress] );
    }
...
{
//	DEBUG_PutString("HOST: _USB_InitControlRead(*pEndpoint, *pControlData, controlSize=0x"); DEBUG_PutHexUINT16(controlSize);	
//	DEBUG_PutString(", *pData, size=0x"); DEBUG_PutHexUINT16(size); DEBUG_PutString(")\r\n");
...
{
//	DEBUG_PutString("HOST: _USB_InitControlWrite(*pEndpoint, *pControlData, controlSize=0x"); DEBUG_PutHexUINT16(controlSize);	
//	DEBUG_PutString(", *pData, size=0x"); DEBUG_PutHexUINT16(size); DEBUG_PutString(")\r\n");
...
{
//	DEBUG_PutString("HOST: _USB_InitRead(*pEndpoint, *pData, size=0x"); DEBUG_PutHexUINT16(size); DEBUG_PutString(")\r\n");
...
{
//	DEBUG_PutString("HOST: _USB_InitWrite(*pEndpoint, *pData, size=0x"); DEBUG_PutHexUINT16(size); DEBUG_PutString(")\r\n");
...
//	DEBUG_PutString("HOST: _USB_NotifyClients(address="); DEBUG_PutDec(address);
//	DEBUG_PutString(", event=0x"); DEBUG_PutHexUINT8(event);	
//	DEBUG_PutString(", *data, size=0x"); DEBUG_PutHexUINT16(size); DEBUG_PutString(")\r\n");
    ...
    switch(event)
    {
        ...
        default:
            ...
            if(usbDeviceInfo[address].flags.bfUseEP0Driver == 1)
            {
                usbClientDrvTable[usbDeviceInfo[address].deviceEP0Driver].EventHandler(address, event, data, size);
            }
            break;
    }
} // _USB_NotifyClients
...
void _USB_NotifyDataClients( BYTE address, USB_EVENT event, void *data, unsigned int size )
{
    ...
    switch(event)
    {
        default:
            ...
            while (pInterface != NULL)  // Scan the interface list for all active drivers.
            {
#ifdef USB_HOST_APP_DATA_EVENT_HANDLER // S.Suwa
                if(pInterface->clientDriver != CLIENT_DRIVER_HUB)
                {
...
/****************************************************************************
  ...
  ***************************************************************************/
// 
BOOL _USB_ParseConfigurationDescriptor( BYTE deviceAddress )
{
	DEBUG_PutString("HOST: _USB_ParseConfigurationDescriptor(deviceAddress="); DEBUG_PutDec(deviceAddress); DEBUG_PutString(")\r\n");
    ...
    error                   = FALSE;
    index                   = 0;
    ptr                     = pCurrentConfigurationDescriptor[deviceAddress];
    currentInterface        = 0;
    currentAlternateSetting = 0;
    pTempInterfaceList      = usbDeviceInfo[deviceAddress].pInterfaceList; // Don't set until everything is in place.

    // Assume no OTG support (determine otherwise, below).
    usbDeviceInfo[deviceAddress].flags.bfSupportsOTG   = 0;
    usbDeviceInfo[deviceAddress].flags.bfConfiguredOTG = 1;

    #ifdef USB_SUPPORT_OTG
        usbDeviceInfo[deviceAddress].flags.bfAllowHNP = 1;  //Allow HNP From Host
    #endif
    ...
    if (!USB_HOST_APP_EVENT_HANDLER( USB_ROOT_HUB, EVENT_VBUS_REQUEST_POWER,
            &powerRequest, sizeof(USB_VBUS_POWER_EVENT_DATA) ))
    {
        usbDeviceInfo[deviceAddress].errorCode = USB_ERROR_INSUFFICIENT_POWER;
        error = TRUE;
    }

    // Skip over the rest of the Configuration Descriptor
    index += bLength;
    ptr    = &pCurrentConfigurationDescriptor[deviceAddress][index];
    ...
    while (!error && (index < wTotalLength))
    {
        ...
        if (bDescriptorType == USB_DESCRIPTOR_OTG)
        {
            ...
            usbDeviceInfo[deviceAddress].flags.bfSupportsOTG = 1;
            usbDeviceInfo[deviceAddress].attributesOTG       = *ptr;

            // See if we need to send the SET FEATURE command.  If we do,
            // clear the bConfiguredOTG flag.
            if ( (usbDeviceInfo[deviceAddress].attributesOTG & OTG_HNP_SUPPORT) && (usbDeviceInfo[deviceAddress].flags.bfAllowHNP))
            {
                usbDeviceInfo[deviceAddress].flags.bfConfiguredOTG = 0;
            }
            else
            {
                usbDeviceInfo[deviceAddress].flags.bfAllowHNP = 0;
            }
        }
        ...
        else
        {
            ...
            if (usbDeviceInfo[deviceAddress].flags.bfUseDeviceClientDriver)
            {
                ClientDriver = usbDeviceInfo[deviceAddress].deviceClientDriver;
            }
            else
            {
                if( Class == USB_HUB_CLASSCODE ) // add S.Suwa  Hub no entry in clientDriverTable[]
                {
                    ClientDriver = CLIENT_DRIVER_HUB;
                }
                else if (!_USB_FindClassDriver(deviceAddress, Class, SubClass, Protocol, &ClientDriver)) // 2014/1/21 S.Suwa
                {
...
//    }
    ...
    {    
        // Set configuration.
        usbDeviceInfo[deviceAddress].currentConfiguration      = currentConfiguration;
        usbDeviceInfo[deviceAddress].currentConfigurationPower = bMaxPower;
    
        // Success!
        DEBUG_PutString( "HOST: Parse Configuration Descriptor success\r\n" );

        usbDeviceInfo[deviceAddress].pInterfaceList = pTempInterfaceList;
        return TRUE;
    }    
}
...
/****************************************************************************
  ...
  ***************************************************************************/
// 
void _USB_ResetDATA0( BYTE deviceAddress, BYTE endpoint )
{
    USB_ENDPOINT_INFO   *pEndpoint;

//	DEBUG_PutString("HOST: _USB_ResetDATA0(deviceAddress="); DEBUG_PutDec(deviceAddress);
//  DEBUG_PutString(", endpoint=0x"); DEBUG_PutHexUINT8(endpoint); DEBUG_PutString(")\r\n");
    ...
    {
        pEndpoint = _USB_FindEndpoint( deviceAddress, endpoint );
        if (pEndpoint != NULL)
        {
            pEndpoint->status.bfNextDATA01 = 0;
        }
...
void _USB_SendToken( BYTE deviceAddress, BYTE endpoint, BYTE tokenType )
{
    BYTE    temp;

/*	DEBUG_PutString("HOST: _USB_SendToken(deviceAddress="); DEBUG_PutDec(deviceAddress);	
	DEBUG_PutString(", ep=0x"); DEBUG_PutHexUINT8(endpoint);
	DEBUG_PutString(",tkn=");
	if(tokenType == 0x0d) DEBUG_PutString("SETUP");
	if(tokenType == 0x01) DEBUG_PutString("OUT");
	if(tokenType == 0x09) DEBUG_PutString("IN");
	DEBUG_PutString(")\r\n");
*/
    ...
    //   when token send to Hub connected low speed device, Clear low speed bits in U1EP0.
    //     PRE_TOKEN automaticaly sending before other token send by USB module.    S.Suwa 
    if (usbDeviceInfo[deviceAddress].flags.bfIsLowSpeed && deviceAddress < 2)
    {
        temp |= 0x80;   // Set LSPD
    }

    // Enable control transfers if necessary.
    if (pCurrentEndpoint[deviceAddress]->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_CONTROL)
    {
        temp &= 0xEF;   // Clear EPCONDIS
    }

    // Disable handshaking for isochronous endpoints.
    if (pCurrentEndpoint[deviceAddress]->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_ISOCHRONOUS)
    {
        temp &= 0xFE;   // Clear EPHSHK
    }

    U1EP0 = temp;
    U1ADDR = usbDeviceInfo[deviceAddress].deviceAddressAndSpeed;
...
void _USB_SetBDT( BYTE deviceAddress, BYTE token )
{
    WORD                currentPacketSize;
    BDT_ENTRY           *pBDT;

//	DEBUG_PutString("HOST: _USB_SetBDT(deviceAddress="); DEBUG_PutDec(deviceAddress);
//  DEBUG_PutString(", token=0x"); DEBUG_PutHexUINT8(token); DEBUG_PutString(")\r\n");
    ...
    if (token == USB_TOKEN_IN)
    {
        ...
        #if (USB_PING_PONG_MODE == USB_PING_PONG__FULL_PING_PONG)
            pBDT = BDT_IN;
            //if (usbDeviceInfo[deviceAddress].flags.bfPingPongIn)
            if (usbDeviceInfo[1].flags.bfPingPongIn)
            {
                pBDT = BDT_IN_ODD;
            }
        #else
        ...
        #if (USB_PING_PONG_MODE == USB_PING_PONG__FULL_PING_PONG)
            //usbDeviceInfo[deviceAddress].flags.bfPingPongIn = ~usbDeviceInfo[deviceAddress].flags.bfPingPongIn;
            usbDeviceInfo[1].flags.bfPingPongIn = ~usbDeviceInfo[1].flags.bfPingPongIn;
        #endif
    }
    else  // USB_TOKEN_OUT or USB_TOKEN_SETUP
    {
        // Find the BDT we need to use.
            ...
            pBDT = BDT_OUT;
            //if (usbDeviceInfo[deviceAddress].flags.bfPingPongOut)
            if (usbDeviceInfo[1].flags.bfPingPongOut)
            {
                pBDT = BDT_OUT_ODD;
            }
        #else
        ...
        // Set up ping-pong for the next transfer
        #if (USB_PING_PONG_MODE == USB_PING_PONG__FULL_PING_PONG) || (USB_PING_PONG_MODE == USB_PING_PONG__EP0_OUT_ONLY)
            //usbDeviceInfo[deviceAddress].flags.bfPingPongOut = ~usbDeviceInfo[deviceAddress].flags.bfPingPongOut;
            usbDeviceInfo[1].flags.bfPingPongOut = ~usbDeviceInfo[1].flags.bfPingPongOut;
        #endif
    }
    ...
    {
        if ((pCurrentEndpoint[deviceAddress]->dataCountMaxSETUP - pCurrentEndpoint[deviceAddress]->dataCount) > pCurrentEndpoint[deviceAddress]->wMaxPacketSize)
        {
            currentPacketSize = pCurrentEndpoint[deviceAddress]->wMaxPacketSize;
        }
        else
        {
            currentPacketSize = pCurrentEndpoint[deviceAddress]->dataCountMaxSETUP - pCurrentEndpoint[deviceAddress]->dataCount;
        }
    }
    else
    {
        ...
        {
            if ((pCurrentEndpoint[deviceAddress]->dataCountMax - pCurrentEndpoint[deviceAddress]->dataCount) > pCurrentEndpoint[deviceAddress]->wMaxPacketSize)
            {
                currentPacketSize = pCurrentEndpoint[deviceAddress]->wMaxPacketSize;
            }
            else
            {
                currentPacketSize = pCurrentEndpoint[deviceAddress]->dataCountMax - pCurrentEndpoint[deviceAddress]->dataCount;
            }
        }
    }
    ...
    if (token == USB_TOKEN_SETUP)
    {
            ...
            pBDT->ADR  = ConvertToPhysicalAddress(pCurrentEndpoint[deviceAddress]->pUserDataSETUP);
        #else
    ...
    else
    {
            ...
            if (pCurrentEndpoint[deviceAddress]->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_ISOCHRONOUS)
            {
                pBDT->ADR  = ConvertToPhysicalAddress(((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->buffers[((ISOCHRONOUS_DATA *)(pCurrentEndpoint[deviceAddress]->pUserData))->currentBufferUSB].pBuffer);
            }
            else
            {
                pBDT->ADR  = ConvertToPhysicalAddress((WORD)pCurrentEndpoint[deviceAddress]->pUserData + (WORD)pCurrentEndpoint[deviceAddress]->dataCount);
            }
        #elif defined(__PIC32MX__)
            if (pCurrentEndpoint[deviceAddress]->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_ISOCHRONOUS)
            {
            ...
            {
                pBDT->ADR  = ConvertToPhysicalAddress((DWORD)pCurrentEndpoint[deviceAddress]->pUserData + (DWORD)pCurrentEndpoint[deviceAddress]->dataCount);
            }
        #else
            #error Cannot set BDT address.
        #endif
    ...
    pBDT->STAT.DTS      = pCurrentEndpoint[deviceAddress]->status.bfNextDATA01;
    pBDT->STAT.DTSEN    = pCurrentEndpoint[deviceAddress]->status.bfUseDTS;

    // Transfer the BD to the USB OTG module.
    pBDT->STAT.UOWN     = 1;

// below debug message exists old version source. S.Suwa

// DEBUG_PutChar('{');
// DEBUG_PutHexUINT8((pBDT->v[0] >> 24) & 0xff);
// DEBUG_PutHexUINT8((pBDT->v[0] >> 16) & 0xff);
// DEBUG_PutHexUINT8((pBDT->v[0] >> 8) & 0xff);
// DEBUG_PutHexUINT8((pBDT->v[0]) & 0xff);
// DEBUG_PutChar('-');
// DEBUG_PutHexUINT8((currentPacketSize >> 24) & 0xff);
// DEBUG_PutHexUINT8((pBDT->v[1] >> 16) & 0xff);
// DEBUG_PutHexUINT8((currentPacketSize >> 8) & 0xff);
// DEBUG_PutHexUINT8(currentPacketSize & 0xff);
// DEBUG_PutChar('}');
...
BOOL _USB_TransferInProgress( BYTE deviceAddress )
{
    USB_ENDPOINT_INFO           *pEndpoint;
    USB_INTERFACE_INFO          *pInterface;
    USB_INTERFACE_SETTING_INFO  *pSetting;

//  DEBUG_PutString("HOST: _USB_TransferInProgress(deviceAddress="); DEBUG_PutDec(deviceAddress); DEBUG_PutString(")\r\n");
...
//      DEBUG_PutChar('~');
      ...
      for(deviceAddress = 1; deviceAddress <= USB_MAX_ADDRESS; deviceAddress++)
      {
            ...
            {
                if(numTimerInterrupts[deviceAddress] != 0)
                {
                    numTimerInterrupts[deviceAddress]--;
...
//      DEBUG_PutChar( '[' );
		...
		// Attach ISR, HUB or first attached device only. 
        if (usbHostState[1] == (STATE_DETACHED | SUBSTATE_WAIT_FOR_DEVICE))
        {
            usbOverrideHostState[1] = STATE_ATTACHED;
        }
...
//    if ((U1IEbits.TRNIE && U1IRbits.TRNIF) && (!(U1IEbits.UERRIE && U1IRbits.UERRIF) || (pCurrentEndpoint->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_ISOCHRONOUS)))
...
//    DEBUG_PutChar( '!' );
      ...
      deviceAddress = U1ADDR & 0x7f;

	  if(deviceAddress == 0){
		for(deviceAddress = 1; deviceAddress <= USB_MAX_ADDRESS; deviceAddress++){
            WORD state = usbHostState[deviceAddress] & STATE_MASK;
			if(state == STATE_ATTACHED || state == STATE_ADDRESSING) break;
		}
	  } 

      if(!(U1IEbits.UERRIE && U1IRbits.UERRIF) || (pCurrentEndpoint[deviceAddress]->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_ISOCHRONOUS))
      {
        ...
        if ((pCurrentEndpoint[deviceAddress]->bEndpointAddress & 0x0F) == (U1TOK & 0x0F))
        {
            ...
            if (pBDT->STAT.PID == PID_ACK)
            {
                ...
                pCurrentEndpoint[deviceAddress]->dataCount += packetSize;

                // Set the NAK retries for the next transaction;
                pCurrentEndpoint[deviceAddress]->countNAKs = 0;

                // Toggle DTS for the next transfer.
                pCurrentEndpoint[deviceAddress]->status.bfNextDATA01 ^= 0x01;

                if ((pCurrentEndpoint[deviceAddress]->transferState == (TSTATE_CONTROL_NO_DATA | TSUBSTATE_CONTROL_NO_DATA_SETUP)) ||
                    (pCurrentEndpoint[deviceAddress]->transferState == (TSTATE_CONTROL_READ    | TSUBSTATE_CONTROL_READ_SETUP)) ||
                    (pCurrentEndpoint[deviceAddress]->transferState == (TSTATE_CONTROL_WRITE   | TSUBSTATE_CONTROL_WRITE_SETUP)))
                {
                    // We are doing SETUP transfers. See if we are done with the SETUP portion.
                    if (pCurrentEndpoint[deviceAddress]->dataCount >= pCurrentEndpoint[deviceAddress]->dataCountMaxSETUP)
                    {
                        // We are done with the SETUP.  Reset the byte count and
                        // proceed to the next token.
                        pCurrentEndpoint[deviceAddress]->dataCount = 0;
                        _USB_SetNextTransferState(deviceAddress);
                    }
                }
                else
                {
                    ...
                    if ((pCurrentEndpoint[deviceAddress]->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_ISOCHRONOUS) ||
                        (packetSize < pCurrentEndpoint[deviceAddress]->wMaxPacketSize) ||
                        (pCurrentEndpoint[deviceAddress]->dataCount >= pCurrentEndpoint[deviceAddress]->dataCountMax))
                    {
                        // We've written all the data. Proceed to the next step.
                        pCurrentEndpoint[deviceAddress]->status.bfTransferSuccessful = 1;
                        _USB_SetNextTransferState(deviceAddress);
                    }
                    else
                    {
            ...
            else if (pBDT->STAT.PID == PID_NAK)
            {
                    ...
                    pCurrentEndpoint[deviceAddress]->status.bfLastTransferNAKd = 1;
                #endif

                pCurrentEndpoint[deviceAddress]->countNAKs ++;

                switch( pCurrentEndpoint[deviceAddress]->bmAttributes.bfTransferType )
                {
                    case USB_TRANSFER_TYPE_BULK:
                        ...
                        if (pCurrentEndpoint[deviceAddress]->status.bfNAKTimeoutEnabled &&
                            (pCurrentEndpoint[deviceAddress]->countNAKs > pCurrentEndpoint[deviceAddress]->timeoutNAKs))
                        {
                            pCurrentEndpoint[deviceAddress]->status.bfError    = 1;
                            pCurrentEndpoint[deviceAddress]->bErrorCode        = USB_ENDPOINT_NAK_TIMEOUT;
                            _USB_SetTransferErrorState( pCurrentEndpoint[deviceAddress] );
                        }
                        break;
                    ...
                    case USB_TRANSFER_TYPE_INTERRUPT:
                        if ((pCurrentEndpoint[deviceAddress]->bEndpointAddress & 0x80) == 0x00)
                        {
            ...
            else if (pBDT->STAT.PID == PID_STALL)
            {
               ...
               // DEBUG_PutChar( '^' );
                ...
                pCurrentEndpoint[deviceAddress]->status.bfStalled = 1;
                pCurrentEndpoint[deviceAddress]->bErrorCode       = USB_ENDPOINT_STALLED;
                _USB_SetTransferErrorState( pCurrentEndpoint[deviceAddress] );
            }
            else
            {
                ...
                pCurrentEndpoint[deviceAddress]->status.bfErrorCount++;

                if (pCurrentEndpoint[deviceAddress]->status.bfErrorCount >= USB_TRANSACTION_RETRY_ATTEMPTS)
                {
                    ...
                    pCurrentEndpoint[deviceAddress]->status.bfError    = 1;
                    pCurrentEndpoint[deviceAddress]->bErrorCode        = USB_ENDPOINT_ERROR_ILLEGAL_PID;
                    _USB_SetTransferErrorState( pCurrentEndpoint[deviceAddress] );
        ...
        // _USB_FindNextToken(deviceAddress);
        _USB_FrameNextTransaction();
    ...
    if (U1IEbits.SOFIE && U1IRbits.SOFIF)
    {
	  ...
	  // Endpoint Interval Count Countdown, in all device
      for(deviceAddress = 1; deviceAddress <=  USB_MAX_ADDRESS; deviceAddress++)
      {
        pInterface = usbDeviceInfo[deviceAddress].pInterfaceList;
        while (pInterface)
...
//      DEBUG_PutHexUINT8( U1EIR );
        ...
        // set error detect device address 
        deviceAddress = U1ADDR & 0x7f;
...
//            _USB_SetNextTransferState();
        ...
        else
        {
            ...
            if (pCurrentEndpoint[deviceAddress]->status.bfErrorCount >= USB_TRANSACTION_RETRY_ATTEMPTS)
            {
                ...
                if (U1EIRbits.BTSEF)
                    pCurrentEndpoint[deviceAddress]->bErrorCode = USB_ENDPOINT_ERROR_BIT_STUFF;
                if (U1EIRbits.DMAEF)
                    pCurrentEndpoint[deviceAddress]->bErrorCode = USB_ENDPOINT_ERROR_DMA;
                if (U1EIRbits.BTOEF)
                    pCurrentEndpoint[deviceAddress]->bErrorCode = USB_ENDPOINT_ERROR_TIMEOUT;
                if (U1EIRbits.DFN8EF)
                    pCurrentEndpoint[deviceAddress]->bErrorCode = USB_ENDPOINT_ERROR_DATA_FIELD;
                if (U1EIRbits.CRC16EF)
                    pCurrentEndpoint[deviceAddress]->bErrorCode = USB_ENDPOINT_ERROR_CRC16;
                if (U1EIRbits.EOFEF)
                    pCurrentEndpoint[deviceAddress]->bErrorCode = USB_ENDPOINT_ERROR_END_OF_FRAME;
                if (U1EIRbits.PIDEF)
                    pCurrentEndpoint[deviceAddress]->bErrorCode = USB_ENDPOINT_ERROR_PID_CHECK;
                #if defined(__PIC32MX__)
                if (U1EIRbits.BMXEF)
                    pCurrentEndpoint[deviceAddress]->bErrorCode = USB_ENDPOINT_ERROR_BMX;
                #endif
...
//***************************************************************************
//***************************************************************************
//*** multiple transfer in one frame, all device ****************************
//***************************************************************************
//***************************************************************************
...
typedef struct {
	BYTE			deviceAddress;
	BYTE			transferType;
} USB_SERVICE_ENDPOINT;

USB_SERVICE_ENDPOINT	ServiceEndpoint[USB_MAX_ADDRESS * 2]; //  2 endpoints/device

#define	USB_FRAME_MAX_TRANSFER_SIZE	1350	// 1200bytes 80%   12M/8=1500byte

static	volatile BYTE	nextSE;
static	volatile BYTE	bnumberOfSE;
...
void	_USB_FindService(BYTE transferType)
{
	...
	for(deviceAddress = 1; deviceAddress <= USB_MAX_ADDRESS; deviceAddress++)
	{
		if(_USB_FindServiceEndpoint(deviceAddress, transferType) == TRUE)
		{
			ServiceEndpoint[bnumberOfSE].deviceAddress = deviceAddress;
			ServiceEndpoint[bnumberOfSE].transferType = transferType;
			bnumberOfSE++;
		}
		if(bnumberOfSE >= USB_MAX_ADDRESS * 2) break;
	}
...
/****************************************************************************
  Function:
	...
	called in SOF interrupt routine
****************************************************************************/

void	_USB_FrameFirstTransaction(void)
{
	// transaction continue?
	if( usbBusInfo.flags.bfTokenAlreadyWritten )
	{
		return;
	}

	// collect service endpoint in all devices.
	bnumberOfSE = 0;
	_USB_FindService(USB_TRANSFER_TYPE_CONTROL);
	_USB_FindService(USB_TRANSFER_TYPE_INTERRUPT);
	_USB_FindService(USB_TRANSFER_TYPE_ISOCHRONOUS);
	_USB_FindService(USB_TRANSFER_TYPE_BULK);

	// not found service endpoint!!	
	if(bnumberOfSE == 0)
	{
		return;
	}
	...
	// clear sent bytes
 	usbBusInfo.wBytesSentInFrame = 0;
...
/****************************************************************************
  Function:
	...
	called in SOF, TRNSF interrupt routine
****************************************************************************/

void	_USB_FrameNextTransaction(void)
{
	do {
		// requested current device transfer TOKEN ?  
		if(_USB_AdvanceFindNextToken(ServiceEndpoint[nextSE].deviceAddress, ServiceEndpoint[nextSE].transferType))
		{
			// wait for TRANSF interrupt
			return;
		}
		// next service endpoint
		nextSE++;
	
	} while(nextSE < bnumberOfSE);

	// all devices transfer end, in now frame
	return;
}
...
/****************************************************************************
  Function:
	void _USB_AdvanceFinedNextoken(BYTE deviceAddress)
	called in SOF, TRNSF interrupt routine
****************************************************************************/
...
BOOL _USB_AdvanceFindNextToken( BYTE deviceAddress, BYTE transferType )
{
    BOOL    illegalState = FALSE;
        ...
        {
			if(usbBusInfo.flags.bfControlTransfersDone) return FALSE;	// one CONTROL Transaction in one frame. 
            ...
            switch (pCurrentEndpoint[deviceAddress]->transferState & TSTATE_MASK)
            {
                case TSTATE_CONTROL_NO_DATA:
                    ...
                    {
                        case TSUBSTATE_CONTROL_NO_DATA_SETUP:
                            usbBusInfo.wBytesSentInFrame += 8;
                            usbBusInfo.flags.bfControlTransfersDone = 1;
                            _USB_SetDATA01( deviceAddress, DTS_DATA0 );
                            _USB_SetBDT( deviceAddress, USB_TOKEN_SETUP );
                            _USB_SendToken( deviceAddress, pCurrentEndpoint[deviceAddress]->bEndpointAddress, USB_TOKEN_SETUP );
                ...
                case TSTATE_CONTROL_READ:
                    switch (pCurrentEndpoint[deviceAddress]->transferState & TSUBSTATE_MASK)
                    {
                        ...
                        case TSUBSTATE_CONTROL_READ_DATA:
                            usbBusInfo.wBytesSentInFrame += pCurrentEndpoint[deviceAddress]->wMaxPacketSize;
                            usbBusInfo.flags.bfControlTransfersDone = 1;
                            _USB_SetBDT( deviceAddress, USB_TOKEN_IN );
                            _USB_SendToken( deviceAddress, pCurrentEndpoint[deviceAddress]->bEndpointAddress, USB_TOKEN_IN );
                            return TRUE;
                    ...
                    case TSTATE_ISOCHRONOUS_READ:
                        switch (pCurrentEndpoint[deviceAddress]->transferState & TSUBSTATE_MASK)
                        {
                            case TSUBSTATE_ISOCHRONOUS_READ_DATA:
                                ...
                                {
                                    if(USB_FRAME_MAX_TRANSFER_SIZE - usbBusInfo.wBytesSentInFrame < pCurrentEndpoint[deviceAddress]->wMaxPacketSize)
                                    {
                                        return FALSE;
                                    }
                                    usbBusInfo.wBytesSentInFrame += pCurrentEndpoint[deviceAddress]->wMaxPacketSize;
            ...
            #endif // SUPPORT ISOCHRONOUS

            #ifdef USB_SUPPORT_INTERRUPT_TRANSFERS
            // Look for any interrupt operations.
            if( transferType == USB_TRANSFER_TYPE_INTERRUPT )
            {
                switch (pCurrentEndpoint[deviceAddress]->transferState & TSTATE_MASK)
                {
                    case TSTATE_INTERRUPT_READ:
